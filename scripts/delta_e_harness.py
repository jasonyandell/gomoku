...
def _play_one_h2h_game(
    fork_picker, parent_picker, *, pair_idx: int, fork_is_black: bool,
    opening_plies: int, seed: int,
) -> str:
    """Play ONE fork-vs-C game from a paired random opening. Returns
    'win'/'loss'/'draw' from the FORK's perspective.

    The opening is regenerated deterministically from (seed, pair_idx), so BOTH
    color assignments of a pair start from the IDENTICAL opening — color bias
    cancels within the pair. This is what makes the deterministic (temperature=0
    argmax) pickers produce INDEPENDENT, balanced games; from the empty board
    they would replay one line per color (2 distinct games → false-precision CI).
    """
    import numpy as np
    ...
    return "win" if winner_side == fork_side else "loss"


def _h2h_tasks(n_games: int, opening_plies: int, seed: int) -> list[tuple[int, bool]]:
    """The (pair_idx, fork_is_black) work list for one match. Each pair emits two
    games (colors swapped, same opening). n_played = 2 * (n_games // 2)."""
    n_pairs = max(1, n_games // 2)
    tasks: list[tuple[int, bool]] = []
    for pair_idx in range(n_pairs):
        tasks.append((pair_idx, True))
        tasks.append((pair_idx, False))
    return tasks


# --- Parallel head-to-head pool (each worker loads BOTH checkpoints once). ---
_H2H_FORK_PICKER = None
_H2H_C_PICKER = None
_H2H_CFG: dict | None = None


def _h2h_pool_init(fork_ckpt: str, c_ckpt: str, sims: int, c_puct: float,
                   device: str, opening_plies: int, seed: int) -> None:
    global _H2H_FORK_PICKER, _H2H_C_PICKER, _H2H_CFG
    _H2H_FORK_PICKER = _build_ckpt_picker(fork_ckpt, sims=sims, c_puct=c_puct, device=device)
    _H2H_C_PICKER = _build_ckpt_picker(c_ckpt, sims=sims, c_puct=c_puct, device=device)
    _H2H_CFG = {"opening_plies": opening_plies, "seed": seed}


def _h2h_pool_play_one(task: tuple[int, bool]) -> str:
    pair_idx, fork_is_black = task
    assert _H2H_FORK_PICKER is not None and _H2H_C_PICKER is not None and _H2H_CFG is not None
    return _play_one_h2h_game(
        _H2H_FORK_PICKER, _H2H_C_PICKER, pair_idx=pair_idx, fork_is_black=fork_is_black,
        opening_plies=_H2H_CFG["opening_plies"], seed=_H2H_CFG["seed"])
...
def head_to_head_eval(
    ...
    *,
    ...
    if use_arena:
        ...
        outcomes: list[str] = []
        with ctx.Pool(n_workers, initializer=_h2h_pool_init, initargs=init_args) as pool:
            for i, o in enumerate(pool.imap_unordered(_h2h_pool_play_one, tasks), 1):
                outcomes.append(o)
                if i % 10 == 0 or i == n_played:
                    w, d, ls = _tally(outcomes)
                    print(f"[delta-e]   {recipe_label}: {i}/{n_played} games "
                          f"({time.perf_counter()-t0:.0f}s) — fork {w}W-{ls}L-{d}D",
                          flush=True)
        wins, draws, losses = _tally(outcomes)
    else:
        fork_picker = _build_ckpt_picker(fork_ckpt, sims=sims, c_puct=c_puct, device=device)
        c_picker = _build_ckpt_picker(c_ckpt, sims=sims, c_puct=c_puct, device=device)
        outcomes = []
        for i, (pair_idx, fork_is_black) in enumerate(tasks, 1):
            outcomes.append(_play_one_h2h_game(
                fork_picker, c_picker, pair_idx=pair_idx, fork_is_black=fork_is_black,
                opening_plies=opening_plies, seed=seed))
            if i % 10 == 0 or i == n_played:
                w, d, ls = _tally(outcomes)
                print(f"[delta-e]   {recipe_label}: {i}/{n_played} games "
                      f"({time.perf_counter()-t0:.0f}s) — fork {w}W-{ls}L-{d}D", flush=True)
        wins, draws, losses = _tally(outcomes)
    ...
    print(f"[delta-e] H2H {recipe_label}: fork {wins}W-{losses}L-{draws}D "
          ...
          f"{time.perf_counter()-t0:.0f}s) -> Δelo={out.delta_elo:+.1f} "
...
def _run_head_to_head(
    ...
    print("[delta-e] head-to-head mode: forks matched directly vs C "
          f"(no anchored-eval of C). parent C: {args.parent}", flush=True)
    ...
    for i, (r, cmd, ckpt_dir) in enumerate(plans, 1):
        ...
        results.append(head_to_head_eval(
            str(final_ckpt), args.parent, recipe_label=r.label,
