...
**Net strategic read:** two levers tried/measured — imitation (DAgger) and more search (deep MCTS) — both fail to cross the Rapfi wall; the wall is an **evaluation** ceiling. Highest-value open directions are now **evaluator-improvement**: #9 (GPU-batched VCF hard tactical truth), #7 (distill Rapfi-NNUE's eval), #2 (solve idx-2). Nothing further built this session pending Jason's read on the eval-lever fork.
