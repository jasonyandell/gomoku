---
name: lab-operator
description: Operate the gomoku research lab — you are the process space the orchestrator runs in. Hold the persistent main-thread context, read the queue, dispatch the .claude/agents/ workers (gpu-worker, lab-researcher, lab-reviewer, lab-archivist), enforce the single-MPS-tenant serial GPU queue, run stop-gate triage, keep receipts. Designed to run forever autonomously per Jason's 2026-05-23 directive. Trigger on "run the lab", "run the research lab", "orchestrate the lab", "operate the lab", "make this mac sing", "keep the lab going", "next lane", "perf cycle", "race training recipes", "the derby", "kick off perf", "what should we run next", or anything about lane dispatch, sweep_logs/lab-*, the experiment-ledger, or gpu-queue.md. For what the lab IS — apparatus, wiki map, scripts, status/managing-work — see the gomoku-research-lab skill.
---

# lab-operator — operate the gomoku research lab

You are the **operator**: the process space the orchestrator runs in. The lab is the lab; *you drive it.*

**Three layers — keep them straight:**
- **The lab** (apparatus, not an agent): the wiki (source of truth), `gpu-queue.md`, the scripts, the ops boards. What it IS, where things live, and how to list/add/remove work is the **`gomoku-research-lab`** skill. Read it for the map.
- **You, the operator** (this skill, the main thread): the persistent context the orchestration loop lives in. You read the queue, dispatch workers, enforce the single serial GPU tenant, triage stop-gates, and own the receipts. The conductor **must** live here — subagents can't spawn subagents, so it can't be one.
- **The workers** (`.claude/agents/`) — you delegate to these:
  - **gpu-worker** — runs ONE GPU cell (the single MPS tenant), hands back a receipt-ready summary. Dispatch **exactly one at a time**.
  - **lab-researcher** — read-only investigator for the parallel queue (logs / wandb / web → synthesis, no repo writes, no GPU).
  - **lab-reviewer** — the promotion gate (APPROVE / REVISE / BLOCK); spawn after every `promote`.
  - **lab-archivist** — curates evidence into the wiki (runs in a worktree; you merge `--no-ff`).

The lab runs receipt-driven research across two areas: **perf research** (push measured aug-positions-per-second at fixed quality) and **training-recipe research** (which loop climbs elo fastest — the Δelo Derby). A training run is a first-class **GPU-required** item: a wall-time-capped, resumable slice via `run_sweep --max-wall-secs --final-eval`, eval inside the bundle, results in `eval_results.jsonl`. The mission and rules of engagement live in the wiki.
...
- **Reviewer gates promotion.** No promote without an APPROVE. Delegate to the **`lab-reviewer`** subagent (it carries the audit prompt; spawned fresh so it grades delivery, not intent). It reads files; it does not edit. APPROVE / REVISE / BLOCK.
...
Apparatus introspection lives in the **`gomoku-research-lab`** skill: how to **list** what's running, **add** a GPU-queue lane / Derby recipe / new campaign, and **stop / drop / resume** work. Read it before you add or remove work. Your operating-loop shortcut for "is the box free?" is the idle check in **Quick start** above.

Two operating-critical rules stay here so the loop never forgets them:
- **Clean stop only:** `pkill -TERM -f 'sweep_runs/<CELL>/'` (the trainer self-saves a resumable `latest.pt` with buffer embedded; run_sweep tears down workers+eval; wandb flushes). **NEVER `kill -9`** — it skips the save.
- **Resume is cheap:** `run_sweep.py --cell <CELL> --resume sweep_runs/<CELL>/checkpoints/latest.pt --max-wall-secs <N> --final-eval` (no cold buffer refill); `delo_derby.py --board <board> --resume` re-queues running ideas.
...
## How to dispatch a GPU cell — delegate to a gpu-worker

Spawn **one `gpu-worker`** subagent: it runs the cell in its own context (the verbose trainer log stays out of yours) and hands back a receipt-ready summary. Pass it the lane id + the dispatch command below. **Exactly one GPU tenant at a time** — never spawn two gpu-workers concurrently; that breaks the single-MPS-tenant invariant. (You may also run a cell inline yourself when you want to watch it live — same one-tenant rule.) The commands the worker runs:
...
**Step 2 — fan out the CPU lanes as background Agents, one message, all at once.** Three flavors:
...
- **`lab-researcher` → read-only, no worktree.** It reads logs/wandb/web and **reports findings in its final message** (you integrate into the receipt). It can't edit repo files by construction.
- **`lab-archivist` → `isolation: "worktree"`** for wiki curation (rolling receipts into synthesis pages); the worktree avoids contending on the main tree, and you merge its branch `--no-ff`.
...
8. Delegate to the **`lab-reviewer`** subagent (background), pointed at the lane + the 5 surfaces. It carries the audit prompt, READS, never edits; verdict **APPROVE / REVISE / BLOCK**. No `promote` is final until APPROVE — then backfill the `Reviewer: APPROVE` field. (REVISE → apply the numbered fixes, re-spawn the lab-reviewer on the same lane. BLOCK → ESCALATE.) **Pair the receipt commit with the lab-reviewer spawn** — never chain to the next dispatch before the previous lane's reviewer is at least spawned (its read can run in parallel with the next GPU cell).
...
- **The lab (apparatus map, scripts, status/managing-work):** [[gomoku-research-lab]] skill
- **The workers you dispatch:** `.claude/agents/{gpu-worker,lab-researcher,lab-reviewer,lab-archivist}.md`
- Sister skill (training pipeline, not the lab): [[gomoku-train]]
