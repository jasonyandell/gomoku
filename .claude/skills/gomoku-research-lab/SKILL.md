...
**RETIRED 2026-07-01 (Jason): the worktree janitor is GONE.** `reclaim_worktrees.py` reclaimed a clean+merged sibling worktree that a LIVE training run was executing from — its PID-lock liveness check never covered arbitrary processes running out of the tree. Worktree cleanup is manual: `ps aux | grep <path>` before any `git worktree remove`. History of the retired design follows.
...
### 2026-06-16 (workflow-master session: #45 instrument built + gated — the cockpit split, scout-first, and worktree-import friction)

**The role was "workflow-master": run + tune workflows, stay lean, let the workflows do the work — the recovery from the prior session's ~400k context-wall.** It held up; the patterns below are the reusable bits.
- **The cockpit split worked cleanly.** A workflow (`.claude/workflows/build-issue-45-white-defense.js`) built the #45 code CPU-only + unit-tested in a worktree → a fresh reviewer that **re-ran the checks** (not just trusted the receipt) → merge-ready branch; the **orchestrator ran the GPU positive-control gate before merge**. Verify-before-promote as *structure*: the workflow stays GPU-free, the science gate is the orchestrator's hands-on job (like the serial merge). Reusable as `build → orchestrator-gates → merge` for any "needs a measurement to validate" issue.
- **Scout-before-build prevented a rebuild** — a read-only scout found ~70% of #45 already existed (`binomial_ci`, the whole `--gate-on white_loss` plumbing). **BUT two caveats:** (a) the scout said two "in-flight" branches were live; they were **stale/merged** (388 commits behind, 0 ahead — the empty `git diff --stat main...branch` is a *merge-base artifact*; use `git merge-base --is-ancestor <branch> main` to tell stale-from-active, not the two-dot stat). (b) the scout's pointer to `mine_validation_archive.py`'s loss-buckets was **stale** (the script changed). The implementer correctly **trusted the code over the brief** and re-sourced the fixture from baseline CPU games. **Lesson: scout to avoid rebuilds, but always tell the implementer "trust the code over this brief where they differ."**
- **Worktree-import friction (recurring sibling of the editable-install leak).** The #45 code lived ONLY in the build worktree (`white_defense.py` isn't in `main` pre-merge) and the `-e` install points `gomoku` at MAIN. To run the new suite from the worktree you need **`PYTHONPATH=<worktree>`** to shadow the editable install, AND `sweep_runs/` (gitignored) exists only in main → use the **absolute main path** for `--checkpoint`. A "run this command" receipt from a build-in-worktree workflow must account for where code + artifacts live *pre-merge*; the build agent's suggested command assumed main paths and wouldn't have run as-is.
- **Read the wiki BEFORE writing the finding.** My first framing ("champion is solid → contradicts the brittle-defender hypothesis") overstated novelty: `white-side-defense-plan.md` Step A (2026-06-15) already had the depth-4(~90% success)-vs-zetor17(0% ) dissociation. The honest finding is a *quantification* + an instrument-design implication (#45 v1 is at the champion's floor → #49 strong-attacker variant), not a contradiction. The wiki is the synthesis layer precisely so a fresh session doesn't re-discover-as-new what's already known.
- **The code-only lane has a floor, and the janitor wasn't wired.** An `implement-backlog` run picked **0/15** (backlog drained of pure-code work; rest is `needs-live-validation`) — an empty code-drain is a *true signal the GPU is the bottleneck*, not a failure. And the session-start janitor never auto-ran (this session continued from compaction; the doctrine says session-start but it's in **no hook** — only `gh_prime.sh` is). Ran it manually (cleaned 4 merged branches). Filed **#47** (manual-sibling coverage gap) + **#48** (wire it into the SessionStart hook). A documented janitor is still a *procedure* until it's actually in the hook.

### 2026-06-16 (workflow-runner session: harden the workflows against API/agent failure — #50)

**An API-overload window (repeated 529 Overloaded) crashed `implement-backlog` with `null is not an object (evaluating 'triage.picks')`.**
- Symptom: during a transient Anthropic-API overload, the triage subagent died on a terminal API error; `agent()` returned `null`; the script dereferenced `triage.picks` with no guard → the WHOLE workflow run aborted with a TypeError. A workflow that turns a network blip into a stack trace is autopilot you cannot trust to run unattended.
- Root cause: the documented `agent()` contract is "returns null when the subagent dies on a terminal API error (after retries) or the user skips it." Every workflow had bare `await agent(...)` results dereferenced without a null-guard: `implement-backlog.js` (`triage.picks`, `verdict.verdict`), `sliding-derby-composite.js` (`score.gate_verdict`), `reviewer-gated-fanout.js` (`verdict.verdict`), `build-issue-45-white-defense.js` (`review.verdict`).
- Fix (#50): a tiny deterministic `agentTry(prompt, opts, tries=3)` helper that re-spawns IDEMPOTENT, read-only agents on null (a fresh re-spawn = a fresh internal-retry budget, riding out a blip), used ONLY on chokepoints (triage / review / score); null-guards everywhere that convert a dead agent into a clean structured outcome (`aborted` / `BLOCK`) + a `log()` line; and the rule **never retry a side-effectful agent** — the composite's train-LAUNCH stays a single bare `agent()` (re-spawning could double-launch a detached GPU slice; it degrades and leans on the re-invocation/re-adoption model instead). Plus `scripts/check_workflow_resilience.mjs`: a no-network smoke test that runs every workflow under agent-death / happy / stage2-death stubs and asserts no-throw (verified RED on the un-hardened files — `reading 'picks'` / `reading 'gate_verdict'` — GREEN after).
- Lesson: a workflow's resilience is a property of its **deterministic JS**, not its agents — the agents *will* die, so the script must degrade. Two simple layers: (1) bounded re-spawn for idempotent chokepoints; (2) graceful degradation + clean exit everywhere else, leaning on re-invocation for the long-outage case. Per the meta-rule above, the GAUGE for this class of entropy is `check_workflow_resilience.mjs` — a deterministic test is how you'd notice the day a workflow regresses back to crashing on a dead agent. Canonical synthesis: `wiki/topics/workflow-orchestration.md` § Resilience.
