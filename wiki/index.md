...
| **Build/operate the autolab** (the self-driving lab: one out-of-git ledger spine + trainer/arena/research/worker loops; epic #53 — spine + daemon + trainer built, P1 #54 / P2 #56 / P3 #57) | [topics/autolab-architecture.md](topics/autolab-architecture.md) | [topics/cockpit-vs-autopilot.md](topics/cockpit-vs-autopilot.md), [topics/research-lab-charter.md](topics/research-lab-charter.md), [topics/m5-max-cross-engine-coupling.md](topics/m5-max-cross-engine-coupling.md). |
...
| [topics/autolab-architecture.md](topics/autolab-architecture.md) | **The autolab (epic #53):** the self-driving lab as one out-of-git append-only ledger spine (`gomoku/lab/{ledger,daemon,trainer}.py`; `~/data/autolab/` home; P1 #54 / P2 #56 / P3 #57 built) read by same-shape loops — trainer (1-epoch→1h singleton slices → HuggingFace per-slice revisions), arena (mac-native gomocup, next), research (ideate→wait-hours), worker (GitHub issues). No-claim flock singleton, the code-shape contract, the measured M5 co-tenancy envelope, the cockpit overlay, and the phased plan. Supersedes the #2 three-tier framing. |
