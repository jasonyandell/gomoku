...
| **Build/operate the autolab** (the self-driving lab: one out-of-git ledger spine + trainer/arena/research/worker loops; epic #53, P1 spine #54 built) | [topics/autolab-architecture.md](topics/autolab-architecture.md) | [topics/cockpit-vs-autopilot.md](topics/cockpit-vs-autopilot.md), [topics/research-lab-charter.md](topics/research-lab-charter.md), [topics/m5-max-cross-engine-coupling.md](topics/m5-max-cross-engine-coupling.md). |
...
| [topics/autolab-architecture.md](topics/autolab-architecture.md) | **The autolab (epic #53):** the self-driving lab as one out-of-git append-only ledger spine (`gomoku/lab/ledger.py`, P1 #54 built) read by four same-shape loops — trainer (1h singleton slices → HuggingFace), arena (mac-native gomocup), research (ideate→wait-hours), worker (GitHub issues). The code-shape contract, the measured M5 co-tenancy envelope, the cockpit overlay, and the phased plan. Supersedes the #2 three-tier framing. |
