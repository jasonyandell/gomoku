...
| **Understand the autolab's *why* / decide if a change fits** (the doctrine: hard walls around a sandbox; no state in a process — reducers over a durable log; deterministic WHEN vs intelligent WHAT; triggers pluggable because not load-bearing; uv as the execution leg; the sim certifies the walls) | [topics/autolab-doctrine.md](topics/autolab-doctrine.md) | [topics/autolab-architecture.md](topics/autolab-architecture.md), [topics/cockpit-vs-autopilot.md](topics/cockpit-vs-autopilot.md). |
| **Build/operate the autolab** (the self-driving lab: one out-of-git ledger spine + trainer/arena/research/worker loops; epic #53 — **LIVE 2026-06-19**: ran 6 real 9×9 slices then a full 15×15 lane unattended, crowned the first 9×9 **and** first 15×15 champion, 0 failures; #65 board-size passthrough, #67 arena artifact-ref fix) | [topics/autolab-architecture.md](topics/autolab-architecture.md) | [topics/autolab-doctrine.md](topics/autolab-doctrine.md), [topics/cockpit-vs-autopilot.md](topics/cockpit-vs-autopilot.md), [topics/research-lab-charter.md](topics/research-lab-charter.md). |
...
| **Design the autolab's research lane** (what researcher-Claude is handed & may emit: the **evidence contract** that fixes the *epistemic* WHEN, the **typed-intent wall** — model proposes meaning, substrate writes rows — the **continuation policy**, **three-zone governance**, the claim-maturity ladder, and the sim invariants that certify them; the #61 design note + the 2026-06 agentic-research literature it mines) | [topics/autolab-researcher-contract.md](topics/autolab-researcher-contract.md) | [topics/autolab-doctrine.md](topics/autolab-doctrine.md), [sources/autolab-agentic-research-lessons-2026-06-20.md](sources/autolab-agentic-research-lessons-2026-06-20.md). |
| **Design the autolab's arena eval lane** (register a model → run the gamut → read a **relative** Elo, the measurement leg of the trainer/researcher/arena triad: the **gomocup-protocol contestant contract** (the clean scaffold, two on-ramps), the **fixed anchor-pinned panel** (not absolute — #35), the honest **two-layer determinism** answer (bit-reproducible nets/baselines + pinned-CI'd time-budgeted engines), the **cached O(panel) baseline**, the `gate`-vs-`panel` job split, and the sim invariants; produces the `arena-verdict` evidence the researcher lane consumes) | [topics/autolab-arena-eval-lane.md](topics/autolab-arena-eval-lane.md) | [topics/autolab-architecture.md](topics/autolab-architecture.md) § Arena-yardstick gap, [topics/engine-panel-derby-design.md](topics/engine-panel-derby-design.md), [topics/autolab-researcher-contract.md](topics/autolab-researcher-contract.md). |
| **DR tabletop — will the autolab survive weeks unattended?** (pull the power at each table of the research→train→eval→research ring: what's durable, what recovers by re-pick, and the few power-pull kinks where an external effect lands before its commit or one torn byte bricks the reducer; the ranked kink list + what the loop sim covers vs. doesn't. 2026-06-24: torn-line tail-guard FIXED + end-to-end `triad_resume_under_crash` scenario BUILT; #83/#84/#85 filed for the design-y rest) | [topics/autolab-dr-tabletop.md](topics/autolab-dr-tabletop.md) | [topics/autolab-arena-eval-lane.md](topics/autolab-arena-eval-lane.md), [topics/autolab-doctrine.md](topics/autolab-doctrine.md) (the sim certifies the cage), [topics/autolab-supervisor-and-monitor.md](topics/autolab-supervisor-and-monitor.md). |
