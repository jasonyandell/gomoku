...
| **idx-2 forward VCT frontier + danger map — "solve the Bruce-Lee board for black"** (2026-06-28: forward-expand idx-2 [white to move] as an AND/OR frontier — **Rapfi-top-8 generates the moves for both sides, the mega GPU VCT solver is the only oracle** [black VCT = win-terminus, white VCT = black-fumble loss-terminus, no minimax/backup]; a deliberately massive approximation, run to learn the reachable shape, NOT a sound solve [the AND-node top-K gap]. **run-a: 9.6M nodes / depth-11 / 90-min wall — TIME-bound, not space**; **throughput dead-flat ~1,750 nodes/s** [bulk-synchronous + the 250-node solver cap clips the tail] ⇒ exponential branching costs exponential *time* at a flat per-node rate, buying ~4 plies past "a handful"; branching decelerates 3.0→1.66× [attacker ~5.1 Rapfi moves, defender ~2.8; terminus frac →39%]; dedup only ~2–3% [near-tree not DAG]. **The 250-node cap hides almost nothing cheap** — <1% of capped flip to a win even at 16× budget ⇒ capped is a genuinely-hard third regime, the 2.4M win harvest is a near-floor. **The danger map** [depths 0–7, 149,627 nodes; append-only, resumable, content-addressed by D4-canonical board]: per-move both-sides danger densities + nearest forced win/loss + honest cap/gap accounting + Rapfi-prior-vs-oracle-danger. idx-2 reads black-favourable [white_threat 0.28 vs black_threat 0.08] but ~half-unknown; danger oscillates with the tempo; Rapfi's mid-ranking is NOT danger-calibrated [its 3rd first-move pick is the most black-dangerous]; low-danger+high-uncertainty ≠ safe [kept separate]. Harvest ≠ backed-up strategy. UI noted, not built) | [topics/idx2-vct-frontier-map.md](topics/idx2-vct-frontier-map.md) | [topics/mega-vct-solver.md](topics/mega-vct-solver.md), [topics/vct-backward-mining.md](topics/vct-backward-mining.md), [topics/rapfi-idx2-distillation-mine.md](topics/rapfi-idx2-distillation-mine.md), [topics/shape-library-engine.md](topics/shape-library-engine.md). |
| **VCT cascade labeler — corpus-scale verdict ledger + at-scale deepening curve** (#97: label EVERY position of the 56.1M-unique rapfi corpus with an exact GPU-VCT verdict via an iterative-deepening node ladder `50→100k`; D4-canonical content-addressed, append-only Parquet, resume-by-row-offset, all proof-outputs ON. **Measured throughput knees** [single-shot, proof-outputs ON]: cap50 **43k b/s @ ~2M width**, cap100 24k, cap250 10k @ 512k, cap500 3.1k @ 64k, cap1000 1.3k @ 64k — width is king till GPU saturation then flat; throughput ~halves per ladder step; knee width shrinks as budget rises. **Live at-scale run [2026-06-30]:** cap50 resolved **87.2%** of the corpus at 50 nodes [48.3% win / 38.9% no_win], 12.8% [7.13M] cap → deeper rungs. **Post-mortem gold:** `timeout`-SIGKILL of MLX mid-Metal-compile wedged the compiler service system-wide and crashed WindowServer — bound GPU runs by WORK, never an external timeout kill) | [topics/vct-cascade-labeler.md](topics/vct-cascade-labeler.md) | [topics/vct-cascade-run-2026-06-30.md](topics/vct-cascade-run-2026-06-30.md), [topics/mega-vct-solver.md](topics/mega-vct-solver.md), [topics/gpu-vct-feasibility.md](topics/gpu-vct-feasibility.md). |
| **⭐ The first VCT arrives at median ply 19 → play self-play TO the VCT, not to five** (measured 2026-06-30 over the full 56.1M-position cascade, #97: joining every game's per-ply positions to the win-ledger, the **first ply where the side-to-move has a forced VCT is median 19 / mean 21.6** [p10–p90 = 12–31], present in **96.4%** of 1.19M games; first-move advantage visible — **68%** of first VCTs are the first player's. This is WHY **48.9%** of all positions are VCT-wins: a VCT exists by ~move 20 [≈10 stones/side] and games run ~40–60 plies, so the whole back half is VCT-saturated. **Actionable headline:** a VCT is a forced win the GPU oracle both *detects and terminally values* [exact win + winning move], so self-play need not play out to an actual five — **terminate at the first VCT and take the oracle verdict**, cutting trajectory length from ~40–60 plies to a median **19** [<½] to a *labeled* winner with an *exact* terminal value instead of a bootstrap ⇒ cheaper + cleaner self-play data, and the game's objective becomes "reach a VCT" = exactly the seek-VCT/L2 target. **AND cap50 is a near-complete first-VCT detector** [measured 2026-06-30]: **98.8%** of all 27.4M VCTs resolve at 50 nodes; a 50-node seeker finds a forced win in **96.1% of all games** [99.7% of vct-games], ~as early as an infinitely-patient solver [median first-VCT ply 20 @ cap50 vs 19 @ cap2000 — 40× budget buys ~1 ply]; only 0.3% of vct-games have a VCT invisible at 50 nodes ⇒ **build the AI as a cap50 mate-seeker** — 40–850× faster iteration, gives up ~nothing; the deep >2000-node tail is a wall of hard-to-confirm no-wins, skip it) | [topics/vct-cascade-run-2026-06-30.md](topics/vct-cascade-run-2026-06-30.md) § first VCT / § 50 nodes | [topics/idea-pile.md](topics/idea-pile.md) #11, [topics/shape-library-engine.md](topics/shape-library-engine.md), [topics/phi-distance-field-learnability.md](topics/phi-distance-field-learnability.md), [topics/vct-reachability-mining.md](topics/vct-reachability-mining.md). |
