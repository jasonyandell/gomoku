...
|---|---|---|---|---|---|
| 2026-05-23 | L09e | **reject (compute-units routing axis null; L09 reject is final)** | 2 cells under live training at the L09 ref shape (small / W=8 / G=8 / S=400 / V=64 / coreml). CPU_AND_GPU = 1,908.3 aug/s (-1.1% vs L09 CPU_AND_NE); ALL = 1,989.8 aug/s (+3.1%). Across-routing spread 4.3% — within noise. ALL is marginal winner. trainer_step_s_p50 across routings: 0.0197-0.0227s (all ~half R-TRAIN-WL5's 0.0512s — MPS-relief real in all 3). | pending Reviewer audit | All three routings still ~40% below R-TRAIN-WL5 (3,297.6). The L09 reject is FINAL: Core ML is genuinely slow at small/V=64 across routings, not "wrong routing". Engine envelope decisively mapped: ANE wins at tiny+V=64 ONLY (L09c +33.9%); 5 measured comparison points across the ANE-axis confirm. consecutive_rejects: 3 → 4 (session-end declared). |
...
- consecutive_rejects: **4** (L09d + L09c-V512 + L08 + L09e — four rejects with clean mechanism: ANE envelope decisively mapped (single-point at tiny+V=64); heap-ratio axis flat; routing axis null). One short of 5-reject HALT threshold; **SESSION-END DECLARED 2026-05-23 by orchestrator** because the four rejects are envelope-mapping with clean mechanism (not knob-failure noise), the ANE-chapter is complete, and remaining queueable lanes have no compound mechanism left to test. Session-end perf-log entry filed; friction-smoothing lessons batched to the gomoku-perf-lab skill. Prior streak: L06-followup-fp16 PROMOTE at R-S400 +97.2%; L06fu-extended × 3 PROMOTEs; L11b' established +152.9% R-TRAIN-LEAN-fp16 perf reference; L09b was blocked-not-reject; L09c PROMOTE (tiny+V=64 on ANE +33.9%); L09d REJECT (medium+V=512 ANE -59.6%); L09c-V512 REJECT (tiny+V=512 ANE -24.0%); L08 REJECT (heap-ratio null at R-S400/fp16); L09e REJECT (routing axis null, L09 reject is final).
...
- **RESUME STATE (session-end 2026-05-23 mid-session-resume, post-L09e)**: L09e closed REJECT (compute-units routing axis null at small/V=64; ALL marginal winner at +3.1% over CPU_AND_NE but still ~40% below R-TRAIN-WL5). The L09 reject is FINAL — not a routing artifact. **Session-end declared.** Five lanes ran this session-portion: L09c PROMOTE (the headline: R-TRAIN-TINY-ANE +33.9%) + L09d/L09c-V512/L08/L09e all REJECT (envelope-mapping with clean mechanism). consecutive_rejects: 4 (one short of 5-reject HALT threshold). **The engine envelope is decisively mapped: ANE wins at tiny+V=64 ONLY (5 measured comparison points across the ANE-axis confirm; V-axis amortization falsified, model-size amortization falsified, compute-units routing axis null, heap-ratio axis null).** L11b' R-TRAIN-LEAN-fp16 +152.9% remains the perf cycle's headline R-TRAIN finding from the prior session-portion. **Next-session queue:** L09f / L09g (downweighted V-axis and model-size sweeps at coreml — predicted rejects per envelope), L09h (.mlpackage re-export cost diagnostic, 1 cell). All Tier 3 / low-upside / diagnostic-only. **THREE NEW friction-smoothing lessons** filed to the gomoku-perf-lab skill at session-end: (1) plies_mean NOT stationary across asymmetric-epoch R-TRAIN cells; (2) session-thermal drift can produce ~5% absolute aug/s drift over 90-min sessions; (3) env-axis lanes should use cells.csv `env` column not shell-prefix env for per-cell artifact stamping. L08 Reviewer APPROVE; L09e Reviewer pending. Class-B housekeeping continues to accumulate (now 5+ new R-TRAIN-* refs + 3 friction-smoothing lessons since session-restart) — pending **user touch** on `wiki/topics/perf-lab-charter.md:50`.
