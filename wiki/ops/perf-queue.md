...
- consecutive_rejects: **3** (L09d + L09c-V512 + L08 — three rejects with clean mechanism: ANE envelope sharply mapped (single-point at tiny+V=64); heap-ratio axis flat in the bandwidth-bound regime). At the warning level per charter; CONTINUE remains correct because Tier-3 lanes are queueable and there's compound mechanism left to test (L09e diagnostic). Prior streak: L06-followup-fp16 PROMOTE at R-S400 +97.2%; L06fu-extended × 3 PROMOTEs; L11b' established +152.9% R-TRAIN-LEAN-fp16 perf reference; L09b was blocked-not-reject; L09c PROMOTE (tiny + V=64 on ANE +33.9%); L09d REJECT (medium + V=512 on ANE -59.6%); L09c-V512 REJECT (tiny + V=512 on ANE -24.0%, V-axis amortization falsified at tiny); L08 REJECT (heap-ratio null at R-S400/fp16, 0.74% spread).
...
- **RESUME STATE (2026-05-23 mid-session, post-L08)**: L08-mps-heap-ratio closed REJECT (axis flat — within-sweep spread 0.74%). consecutive_rejects: 2 → 3 (warning level; CONTINUE per charter because Tier-3 lanes still queueable). **Top-of-queue for next dispatch:** **L09e** (compute-units routing sweep, Tier 3, priority 3.0, ~16 min wall, 6 cells) — the only remaining diagnostic that could rescue any ANE result (distinguishes "ANE just slow at medium" from "Core ML demoted ops"). All remaining queueable lanes are diagnostic or low-upside (L09f priority 2.5, L09g 2.0, L09h 1.0 — all downweighted by L09c-V512). **The session's headline is intact: L09c PROMOTE +33.9% (R-TRAIN-TINY-ANE)**; the 3 rejects (L09d, L09c-V512, L08) sharply map the engine envelope around that single-point win. **NEW friction-smoothing lessons:** (1) plies_mean NOT stationary across asymmetric-epoch R-TRAIN cells; (2) session-thermal drift can produce ~5% absolute aug/s drift over 90-min sessions — back-to-back A/B remains reliable, cross-time comparisons should re-measure. L08 Reviewer pending. Class-B housekeeping continues to accumulate (now 5+ new R-TRAIN-* refs since session-restart) — pending **user touch** on `wiki/topics/perf-lab-charter.md:50`.
