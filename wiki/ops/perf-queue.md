...
- **RESUME STATE (session-end 2026-05-23 mid-session-resume, post-L09e)**: L09e closed REJECT (routing axis null at today's Core ML; ALL marginal winner at +3.1% over CPU_AND_NE but still ~40% below R-TRAIN-WL5). The L09 reject stands **at the current Core ML + evaluator pipeline + model-arch family** — not a permanent verdict. **Session-end declared at a complete current-state ANE snapshot.** Five lanes ran this session-portion: L09c PROMOTE (the headline: R-TRAIN-TINY-ANE +33.9%) + L09d/L09c-V512/L08/L09e all REJECT (envelope-mapping with clean mechanism). consecutive_rejects: 4 (one short of 5-reject HALT threshold). **Current engine envelope snapshot: ANE wins at tiny+V=64 ONLY at today's stack** (5 measured comparison points across the ANE-axis confirm; V-axis amortization currently doesn't compound at tiny, model-size amortization currently doesn't extend to small/medium, routing axis null, heap-ratio axis null). **Future-shape:** Jason flagged inbound new ANE research; when it lands (new Core ML version, new ANE features, evaluator-pipeline updates), re-run L09c/L09d/L09c-V512/L09e against the new baseline before assuming today's snapshot holds. The single-point envelope is the right reading of today's stack, not a structural ANE limit. L11b' R-TRAIN-LEAN-fp16 +152.9% remains the perf cycle's headline R-TRAIN finding from the prior session-portion. **Next-session queue (paused, not closed):** L09f / L09g (V-axis and model-size sweeps at coreml — currently downweighted, but the new ANE research may make them load-bearing again), L09h (.mlpackage re-export cost diagnostic, 1 cell). **THREE NEW friction-smoothing lessons** filed to the gomoku-perf-lab skill at session-end: (1) plies_mean NOT stationary across asymmetric-epoch R-TRAIN cells; (2) session-thermal drift can produce ~5% absolute aug/s drift over 90-min sessions; (3) env-axis lanes should use cells.csv `env` column not shell-prefix env for per-cell artifact stamping. L08 + L09e both Reviewer APPROVE. Class-B housekeeping continues to accumulate (now 5+ new R-TRAIN-* refs + 3 friction-smoothing lessons since session-restart) — pending **user touch** on `wiki/topics/perf-lab-charter.md:50`.
