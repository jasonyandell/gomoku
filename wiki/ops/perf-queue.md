...
| **R-S200** | small / W=8 / G=8 / S=200 / **V=512** | **9,156 aug/s** (L03) | **+52.5%** (fp16 re-measure queued) |
| **R-S100** | small / W=8 / G=8 / S=100 / **V=512** | **15,082 aug/s** (L03) | **+35.2%** (fp16 re-measure queued) |
...
- **RESUME STATE**: **L06-followup fp16-eval is the session's headline win** — R-S400 nearly doubles (4,765 → 9,398.5), R-S400-tiny gets +3.6% (22,088 → 22,874). consecutive_rejects RESET to 0. Stop signal CLEARED. Next compound lanes (in priority order): **L06fu-extended** (fp16 at R-S200 / R-S100 / medium-model — the bandwidth-bound regime should compound), **L09b** (revisit ANE + fp16 workers — fp16 might halve the L09 worker-side loss), **L11b'** (fp16 + V=512 + low sgd at trainer level — could compound with L11b's +28% aug/s finding). Also: **L08-mps-heap-ratio** (3-cell env-var sweep at the new fp16 reference). Note the 4-Reviewer charter-staleness flag on the R-TRAIN-LEAN row (charter:50) still needs user touch.
