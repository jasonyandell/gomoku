...
status: queued (Tier 1-equivalent priority despite Tier 3 label — it's the rescue lane for the entire ANE strand)
notes: Positive control: `coreml_ane_residency_scout.py` "v3_gomoku_fixed_fused_fp16" export hit the ANE rail 2026-05-22 (baselines.md). Negative: `coreml_evaluator.export_model_to_coreml` (convert_to="mlprogram") runs on CPU/BNNS per L09e'. Diff the ops. Per hollance, the mlprogram converter prefers ND-broadcastable ops that fall off the ANE — likely culprit. See [coreml-design-envelope-and-our-fit.md § L09i](../topics/coreml-design-envelope-and-our-fit.md#l09i--mlpackage-op-inspection-coreml-queue--code-no-gpu-needed) and [m5-max-cross-engine-coupling.md](../topics/m5-max-cross-engine-coupling.md) Part 1.
```
