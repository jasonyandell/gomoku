...
- consecutive_rejects: **0** (RESET — L06-followup-fp16 promoted at R-S400 +97.2% and R-S400-tiny +3.6%; stop signal CLEARED). The session that nearly halted instead found its headline win.
...
- **RESUME STATE**: **L11b' is the perf cycle's biggest win** — R-TRAIN family doubles to 8,340.5 aug/s (+153% vs WL5 baseline 3,297.6); compound of L11b (low-sgd, trainer-side) + L06-followup (fp16, worker-side); two independent levers stacked multiplicatively. Compound follow-up dispatching now: **L09b** = R-TRAIN-ANE + fp16 workers. If fp16 halves the worker-side ANE loss from L09 (small/V=64 gen 2× slower on ANE than MPS), and the trainer-side ANE-relief gain is still real (it was -56% trainer_step_s_p50), R-TRAIN-ANE might finally pay. Lower-priority follow-ups: L11b'' (sweep sgd_per_position at V=512+fp16 for the optimal trainer-work band); L06fu-medium-AB (clean medium V=512 fp32 vs fp16 attribution; estimated fp16-only +62%). The R-TRAIN-LEAN charter row at charter:50 is now stale on TWO axes (V=128 reference AND post-L11b' results unknown) — 5+ Reviewer flags by now; needs user touch.
