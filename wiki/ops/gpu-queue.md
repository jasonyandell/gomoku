...
> tenants and waits, so it coexists with a hand-launched derby during
> migration. This markdown stays the **human narrative + RESUME STATE**;
> the daemon's `pending/` is the machine-readable queue. Migration
> (derby → client, markdown → feed) is staged in the topic page.
...
- **RESUME STATE (2026-05-24, Δelo Derby v3 RE-RESUMED then STOPPED — "resume at 5m intervals" → "stop the training for now"; disk hit 100% during teardown)**: Jason asked to resume v3 (continue the existing contenders toward their full 2h/idea wall cap; engine = production multiprocess `run_sweep_wall_slice`, NOT the v1 single-process trap) with a 5-min cron narrator/watchdog (cron `9caaa0b2`, now deleted) instead of 10-min. Ran to 22 chunks (forced went 1149→1168; sims100 was mid-chunk) then Jason said stop. **Teardown lessons (logged in skill friction log):** `pkill -TERM -f delo_derby` killed ONLY the scheduler, not its child `run_sweep`/trainer/8 workers (their command lines lack "delo_derby") → the sims100 chunk orphaned and kept running; had to SIGTERM `gomoku.train.*derby-sims100` + `selfplay_worker.*derby-sims100` + `eval_worker.*derby-sims100` directly. Mid-teardown the **Data volume hit 100% (ENOSPC)** — even the Bash tool's own output file couldn't be written, blocking all commands. Freed ~21G via `rm -rf sweep_runs/derby-*/checkpoints/_records` (transient, already-ingested game shards; regenerated on resume) → 24Gi free / 99%. **STILL TIGHT — big consumers remaining (awaiting Jason's call):** `sweep_runs/derby_v3/_peaks` 80G (per-idea best checkpoints; v3 verdict already marked the run's checkpoints "abandoned"), `WL5-diagnostics-archive-start` 32G, `LF1-lean-fp16-canary` 17G, `WL3-random-openings.dead-e825` 9G (`.dead` = already marked deletable). Derby is RESUMABLE later via the documented command IF per-cell `latest.pt` is kept. (NOTE to next session: an earlier RE-RESUMED entry I wrote here got reverted by a concurrent IDE/file edit — parallel-edit hazard.) consecutive_rejects: n/a (research framework).
- **RESUME STATE (2026-05-24, Δelo Derby v3 CALLED — fixed-step trainer + Gumbel@100 win; board cleared for new contenders)**: Jason called v3 once `gumbel-fast5s` proved itself, in prep for a new round. **WRITTEN UP: research-board.md § "v3 FINAL — verdict"** (full standings + 5 findings + build-artifacts + caveats + next-round candidates). **Verdict:** Gumbel cheap-sims generator dominates (gumbel/gumbel-fast5s peak ~1580–1620 ✓ vs the sims100 plain-MCTS@100 control 697 — Gumbel RESCUES cheap sims, 2×+); the **fixed-step trainer (gumbel-fast5s: non-wave + `--sgd-steps-per-epoch 64`) is CO-EQUAL with wave** — peaks within eval noise (1620 vs 1580) and on the canonical Δelo/hr (standings.md) wave EDGES it 3031 vs 2825, so NOT a rate separation (an earlier "~2× beats wave" draft was a Reviewer-caught overclaim — corrected); fixed-step's wins are STRUCTURAL (peak in one contiguous slice + structurally can't run away vs sgd_per_position + validated the gen-flooding fix → the cleaner v4 control), reuse ~1.4→1.05 + pl 4.39→1.72 descending = productive-not-redundant; both ≫ v1's exploration champion open-div4 (776) → "exploration beats compute" SUPERSEDED once you have good cheap targets + efficient training; forced (KataGo) solid 3rd (1262); regime correction validated (gen FLOODS the trainer, gen=0.4s≪train=3.0s). Derby STOPPED (idle); the run's checkpoints abandoned (writeup uses the captured snapshot). **Co-leaders gumbel vs gumbel-fast5s are within eval noise on ceiling (~1600, anchor ladder saturates ~1700) → a head-to-head is the rigorous tiebreak if ever needed (NOT run).** **Built this round (all merged+tested):** native Gumbel C port, fixed-step trainer mode, wave-SIGTERM deadlock fix(+test), Δelo/hr hill-climb scheduler (+peak tiebreaker), watch_derby.py + derby_dashboard.py + derby_sync_elo.py, the eval-already-in-wandb discovery, the gen-flooding correction. **NEW CONTENDERS for the next round (board clear):** parked **aux-head** (`feat/v3-auxhead` @ 510a9b3, opponent-reply, Class-C, awaiting sign-off); **N-sweep on `--sgd-steps-per-epoch`** (the reuse knob — how hard can fixed-step push before redundant SGD?); reanalyze/curator (need train_replay flywheel → own board); `gumbel-fast5s` is the **new baseline to beat**. Promotion of fixed-step+Gumbel → a production lineage is a deliberate ESCALATE (Jason's call), deferred. **TOOLING NOTE:** the full-buffer (1.5M) clean-save on SIGTERM takes >15s, so a 15s stop-timeout SIGKILLs mid-save — bump the derby stop-timeout for full-buffer cells (the deadlock fix itself works; my stop-timeout was just too short). **Reviewer: REVISE → ACTIONED** — caught the "~2× Δelo/hr / fixed-step beats wave" overclaim (it inverted the derby's own standings.md, where wave's canonical Δelo/hr 3031 > fast5s 2825); corrected to "co-equal, structural wins" across all 4 surfaces 2026-05-24. Reviewer also verified: jsonl peaks (1620/1580/1262/776/697/603) all match, regime-correction holds (gen≪train), exploration-superseded fair (open-div4 ran equal-or-less budget), productive-SGD mechanism solid. consecutive_rejects: n/a (research framework).
- **RESUME STATE (2026-05-24, Derby v3 RUNNING clean — Δelo/hr hill-climb priority + wandb dashboard tooling)**: Since the unified-launch entry below, three things landed. **(1) Clean-restarted v3:** the derby cell dirs held stale v2/earlier state (carryovers' latest.pt predated launch; eval_results.jsonl spanned hours) → asymmetric contamination of the lever comparison. Stopped v3, `rm -rf` all 9 derby-* cell dirs + sweep_runs/derby_v3, relaunched from empty → all 9 cells fresh. (Other experiments WL*/az-recipe untouched.) **(2) wandb tooling (3 committed scripts):** `scripts/watch_derby.py` (terminal board: wandb dynamics + eval_results.jsonl elo, `--watch N`); `scripts/derby_dashboard.py` (native wandb WORKSPACE — regex-filters to the 9 clean v3 runs, greys out stale dupes, colors each, 6 stat sections; URL https://wandb.ai/jasonyandell-forge42/gomoku?nw=cgw5ktj48x9 — re-run to refresh/add runs as gumbel/sims100 start); `scripts/derby_sync_elo.py` (pushes model_elo+peak from eval_results.jsonl into each run's wandb SUMMARY — safe since trainer is --no-eval so eval/* never conflicts; re-run to refresh the elo leaderboard columns). model_elo is NOT a wandb history metric (trainer --no-eval); the dashboard shows it via summary/table. **(3) Scheduler priority CHANGED (commit db5f5c7):** was v2 current-elo "feed the leader"; NOW per Jason "never-run, then Δelo/hour — hill climb elo." `pick_priority`: (a) entry-fee group = ideas with <2 elo points (no measurable slope) rank first, fewest points first → drains round-0 THEN round-1 so every lever establishes a Δelo/hr slope; (b) then last-chunk Δelo/hr descending. `delo_per_hr()` returns None for <2 points (fixed a floor-noise artifact: at round-0 all sit at ~389 floor, (elo-floor)/wall went slightly negative and mis-fed sims100). Applied via stop + `--resume` (state preserved; stops needed SIGKILL on the recurring trainer-deadlock-on-SIGTERM — state is atomic on disk so resume is safe). Currently in round-1 (all 9 at pts=1, entry-fee, each getting its 2nd chunk). Driver log path in `$CLAUDE_JOB_DIR/derby_v3_log_path`; watcher armed. **TO RESUME the derby after any stop:** `python scripts/delo_derby.py --board scripts/derby_v3_board.json --resume`. Post-v3 analysis (head-to-head leaders, 5-surface receipt, aux-head own-board) unchanged from below.
- **RESUME STATE (2026-05-24, Δelo Derby v3 UNIFIED board LAUNCHED — v2 ported in + all prior-art levers merged + native rebuilt)**: Jason: "port these v2 experiments over to v3 and just start working v3." Done. **Stopped v2** at round-0 (all at 389 floor → zero data lost; freed the box for the native rebuild). Stop needed a SIGKILL on one orphaned trainer (deadlocked on a dead-worker wave barrier after I killed orchestrator+workers together; justified -9 since v2 was abandoned + fresh-start v3 doesn't need its ckpt — see friction log). **Merged ALL code lanes to main** (each opt-in, production byte-identical-when-off, verified): playoutcap (b498426), gumbel-python (05becb8), swa, forced-playout (b5e6a66, native+python), gumbel-cport (47498c2, native wall-fair). **The `_mcts_native.c` conflict** (forced + gumbel both extend NativeMCTSGame_init) resolved by merging both kwarg sets into one kwlist + PyArg format `"|OddddKdiidd"` (forced_playout_k then 4 gumbel args); **ONE rebuild** `python setup.py build_ext --inplace`; verified: constructor accepts both, `has_native_gumbel()=True`, default-path deterministic, 34+ tests green, no worker-lever leak into the trainer cmd. **LAUNCHED Derby v3** (`scripts/delo_derby.py --board scripts/derby_v3_board.json`, `pgrep -f delo_derby`, DETACHED nohup — NOT harness-tracked; driver log path in `$CLAUDE_JOB_DIR/derby_v3_log_path`, standings `sweep_runs/derby_v3/standings.md`). **9 ideas, one lever each vs `derby-c0` control**: c0, open-div4/temp-16/sgd-800 (v2 carryovers), playoutcap (--playout-cap-frac 0.25 --playout-cap-fast-sims 50), forced (--forced-playout-k 2.0), swa (--swa-window 5, trainer-side), gumbel (native --gumbel-root @sims=100), sims100 (plain @sims=100, gumbel's control). All `derby-*` cells in run_sweep.CELLS; flags route via extra_worker_args (gen levers) / extra_train_args (swa). slice=600s, cap=7200s/idea, ~108 chunks/18 wall-hr to full cap BUT feed-the-leader + CALL EARLY when decisive (v1 was called at 5/8). Round-0 = c0 fresh. **Reviewer APPROVE** (byte-identical-OFF verified on all 4 levers in code AND live — confirmed the running derby-c0 workers carry NO v3 flags, production base; the `_mcts_native.c` format string `"|OddddKdiidd"` = 11 conversions match 11 arg ptrs + kwlist in order; .so rebuilt post-merge; no production-default flip (additive diff, run_sweep zero removed lines); 67 passed/1 skip. Caught + cleared a real risk: opp_picker stays None so the playoutcap/gumbel flags are genuinely honored on the recorded self-play path, not silently routed to generate_games_vs_baseline). **A v3-completion watcher is armed** (background bash exits when delo_derby ends → re-invokes me). **PARKED for follow-up:** aux-head (`feat/v3-auxhead` @ 510a9b3, opponent-reply head, byte-identical-off + D4-test-passes + inference-drop, Class-C) → merge + give its OWN board (its own axis). **NEXT on v3 cap (or early-call):** read standings, head-to-head the leaders (delta_e_harness --head-to-head), file the v3 receipt across the 5 surfaces, backfill Reviewer APPROVE. consecutive_rejects: n/a. See [[project-delo-derby]], [[project-perf-bench-lesson]].
- **RESUME STATE (2026-05-24, Derby v3 — 3 of 4 levers MERGED to main; forced-playout + gumbel-C-port HELD for post-v2 native rebuild)**: All 5 v3 lanes returned. **MERGED to main (byte-identical-when-off, all verified — 34 tests green, default-path determinism confirmed, derby survived every merge and rolled to its temp-16 chunk on the merged code):** (1) `feat/v3-playoutcap` @ b498426 → merged (pure orchestration, native-fast, wall-fair); (2) `feat/v3-gumbel` @ 2e50fdb → merged commit 05becb8 (NEW gomoku/gumbel.py, **PYTHON path only ~5× slower than native PUCT** — correct target-quality lever but NOT wall-fair; C-port spec embedded in gumbel.py); (3) `feat/v3-swa` @ 75dcd45 → merged (train.py worker_weights publish, --swa-window, SWA-replaces-EMA, wall-fair). Merge hygiene: gumbel conflicted with playoutcap in selfplay_worker.py argparse (resolved = keep both blocks) + generate_games call (auto-merged, both levers' kwargs threaded; dispatcher checks gumbel_root first→python path, else native+optional-PCR; no-flags default = original _generate_games_native). **HELD (NOT merged — both rebuild `_mcts_native.so`, which is gitignored, so merging the .c while the derby runs would leave main's .so STALE and the derby's next chunk would crash passing the new kwarg to the old constructor):** (4) `feat/v3-forcedplayout` @ e91f7d5 — native-C (rebuilt, byte-identical vs orig .so across 4 seeds) + Python fallback, flag --forced-playout-k (wall-fair); merge post-v2. (5) **gumbel-C-port DONE + verified, HELD** `feat/v3-gumbel-cport` @ bf30d97 — ported gumbel into _mcts_native.c (per-game Sequential Halving inside the lockstep wave; root logits retained for completed-policy target). **WALL-FAIR CONFIRMED: native-gumbel = 0.86–1.26× native-PUCT ms/game (the 5× Python DOA is gone; sometimes FASTER than PUCT since SH caps total sims at budget), 3–4× faster than the python path.** Byte-identical OFF across 384 cases (0 mismatch vs orig .so); target parity vs python ref EXACT (0.0 diff, 180 cases); 176 tests pass. Merge post-v2 alongside forced (both touch _mcts_native.c → resolve C conflict then ONE rebuild + verify; complementary: forced=PUCT root top-up vs gumbel=SH root forcing, each gates on own OFF-default kwarg). **DESIGN, awaiting Jason:** `feat/v3-auxhead-design` @ d50a4b2 (wiki/topics/auxiliary-targets-design.md, Class-C, opponent-reply head recommended, 4 open questions; recommended defaults: return_aux kwarg / lazy-alloc / own board / weight 0.15→sweep). **POST-V2 BATCH (when Derby v2 caps — it is a DETACHED nohup, NOT harness-tracked, so check `pgrep -f delo_derby` / sweep_runs/derby_v2/standings.md):** on idle box → merge forced-playout + gumbel-C-port, resolve _mcts_native.c conflict, ONE `python setup.py build_ext --inplace`, verify (byte-identical-when-off + 4 seeds + tests) → assemble `scripts/derby_v3_board.json` + run_sweep cells (playoutcap/forced/swa/gumbel-native all wall-matched vs C0; title cards already in research-board.md §v3) → spawn ONE Reviewer over the integrated v3 code + board → launch Derby v3. Commit 27ebb48 = v3 title cards + this session's RESUME STATE history. consecutive_rejects: n/a (research framework).
- **RESUME STATE (2026-05-24, Δelo Derby v2 LAUNCHED + Derby v3 prior-art lanes fanned out — "learn from those who came before")**: Jason asked for more training ideas mined from other implementations (the Gomocup engine survey — Rapfi/KataGomo/AlphaGomoku(MK) — + KataGo/Gumbel/Leela/EfficientZero literature) and to integrate them via the lab. Decisions (AskUserQuestion): race ALL FOUR search/recipe levers as Derby v3; sequence PARALLEL (v2 on GPU lane + build v3 code now); BLESS the aux-targets Class-C arch lane at DESIGN stage only. **GPU LANE: Δelo Derby v2 RUNNING** — `pgrep -f delo_derby` (pid ~86731), board `scripts/derby_v2_board.json`, top-3 head-to-head (open-div4/temp-16/sgd-800) on the PRODUCTION multiprocess recipe (run_sweep wave-mode, 1 trainer + 8 selfplay_workers, GPU-saturated → wall-clock finally HONEST). slice=600s, cap=7200s/idea, ~36 chunks / ~6 wall-hr, --final-eval in-bundle; standings → `sweep_runs/derby_v2/standings.md`, peaks → `sweep_runs/derby_v2/_peaks/`; driver log path in `$CLAUDE_JOB_DIR/derby_v2_log_path`. Round-0 = open-div4 fresh. **EVERYTHING-ELSE LANE: 5 background worktree agents fanned out for Derby v3 levers** (each: opt-in flag, production byte-identical, merge-local-main-first, PYTHONPATH-not-pip-install-e, CPU-only tiny smoke, branch `feat/v3-<lane>`, commit DON'T merge — I merge --no-ff serially): (1) **v3-playoutcap** — Playout-Cap Randomization (KataGo), pure orchestration in self_play.py `_generate_games_native`, flags `--playout-cap-frac`/`--playout-cap-fast-sims`; (2) **v3-swa** — SWA tail-averaging of generator weights (Leela), train.py worker_weights publish, flag `--swa-window` (fixes v1's ema-099 floor: EMA lagged the learner; SWA = flat tail, no unbounded lag); (3) **v3-gumbel** — Gumbel root + sequential halving (DeepMind 2022), HIGHEST-leverage vs our generation-bound finding (provably good targets at n=2..16), needs `_mcts_native.c` change for fast path OR python-mcts + C-port spec, flags `--gumbel-root`/`--gumbel-m`/`--gumbel-c-visit`/`--gumbel-c-scale`; (4) **v3-forcedplayout** — Forced playouts + policy-target pruning (KataGo), native-C forcing + python pruning, flag `--forced-playout-k`; (5) **v3-auxhead-design** — DESIGN-ONLY Class-C, writes `wiki/topics/auxiliary-targets-design.md` (opponent-reply aux head recommended; no model.py edit, no merge — for user sign-off). **NEXT (when lanes return):** merge each --no-ff serially → 4-point byte-identical check (--help shows flags / default --dry-run emits none / combined threads all / tests green) → assemble `scripts/derby_v3_board.json` + run_sweep cells (one lever vs C0 base, same shape as v2) + research-board.md v3 title cards → launch Derby v3 when v2 caps. Gumbel/forced-playout may come back python-path-only (assess C feasibility) → decide native port then. consecutive_rejects: n/a (research framework). See [[project-delo-derby]], [[feedback-lab-scheduler]], [[project-perf-bench-lesson]].
- **RESUME STATE (2026-05-24, Δelo Derby v1 CALLED + written up; v2 queued — top-3 head-to-head on the PRODUCTION multiprocess recipe)**: Jason called the derby at 5/8 capped (decisive science in). **FINAL ranking (by ELO, wall-clock BUSTED — see caveat): open-div4 (random openings) 1385 > temp-16 (temperature) 1340 > sgd-800 1284 > sims-400 1265 > buf-30k 908 > floor (C0/ema/sims-100).** Headline: **exploration/diversity levers beat raw compute for the ceiling**; compute levers grok faster (beat-heur ep50/70 vs ep90) but peak lower + overtrain ~ep90; open-div4 ended AT peak (no overtrain); sims-100 (100 sims) never grokked. We're **generation-bound** (train = fixed ~10.5s/ep floor; MCTS gen 2-5×). **WALL-CLOCK CAVEAT: the v1 derby ran SINGLE-PROCESS (one gomoku.train, GPU ~30%, ~70% machine idle)** — Jason caught it — so all v1 wall-times/Δelo/hr are single-stream + unrepresentative ([[project-perf-bench-lesson]]); ranked by elo only. **The production recipe IS multiprocess (verified run_sweep.py:517 — 1 trainer + 8 selfplay_workers, wave-mode); drift was only in the derby harness.** Written up: research-board.md (v1 FINAL + v2 plan), experiment-ledger (receipt), perf-log (narrative). 15-min cron 8f889081 DELETED. Mid-run fixes: priority→current-elo (was last-Δelo, fed the worst idea, 76c4d73); wall-time recording added (0b13688). **V2 (queued, NOT yet built): re-run top-3 (open-div4/temp-16/sgd-800) HEAD-TO-HEAD using run_sweep wave-mode (saturated → real wall-clock); eval = round-robin direct matches (all >1280 → saturate anchor ladder → head-to-head correct); WALL-NATIVE budget (hours not epochs; chunk=wall-slice; allocate+stop by Δelo/hr); carry fixes (current-elo priority, peak-checkpoint snapshotting). Build = a new harness or extend delo_derby to drive run_sweep cells + wall-slice chunks.** See [[gomoku-delo-derby]], [[autonomous-overnight-ops]].

- **RESUME STATE (2026-05-24, Δelo DERBY launched — autonomous overnight race of 8 training recipes to 140 epochs; Reviewer APPROVE)**: Jason's "race to 140" idea — race fresh-start self-play recipes, feed compute to the fastest climber, everyone reaches 140 (leaders first). Built `scripts/delo_derby.py` (crash-resumable scheduler, Δelo-rate priority queue, per-chunk failure isolation) + `scripts/derby_board.json` (8 ideas, one lever vs C0-baseline: sims-400/100, sgd-800, buf-30k, open-div4, ema-099, temp-16) + `wiki/ops/research-board.md` (title cards + auto-standings). Chunk = `gomoku.train --no-eval --epochs 10 [--resume]` THEN one-cycle `gomoku.eval_worker` (trainer alone lacks model_elo). Commit 4d887be. **Reviewer APPROVE** (traced no-starvation, epoch-math no-drift, crash-resume, failure isolation, elo parsing, all-evals-fail-still-progresses, board atomicity; 2 cosmetic nits — harmless 1-chunk overshoot window, roundabout beat_before). RUNNING: pid via `pgrep -f delo_derby`, base `sweep_runs/derby/`, driver log `sweep_runs/derby/derby.driver.log`, state `derby_state.json`, milestones `derby_milestones.log`, wandb runs `derby-<idea>` (one per idea, run-id inherits on resume). ~112 chunks total (~9-15h); leaders-first so champion + top contenders land first. **15-min status cron `8f889081`** (`7,22,37,52 * * * *`, session-scoped) reports + PushNotifies + watchdog-relaunches the scheduler if dead; on completion it files the derby results receipt + deletes itself. Operated per [[autonomous-overnight-ops]] memory. NEXT-BOARD ideas (need train_replay flywheel, not fresh self-play): curators (recency/lru/diversity/trouble-pinned), gomocup-curriculum-seed. consecutive_rejects: n/a (research framework).
- **RESUME STATE (2026-05-24, delta-e run-2 DONE — head-to-head resolves the curator question: recency_weighted >> lru; method validated)**: Run-2 completed (pid 29082, sweep_logs/delta-e-run2-20260524T044710Z/). **The head-to-head fix worked: CI ±218 (run-1) → ±76 (run-2), ~3× tighter; 2/3 verdicts now "signal" (run-1 was all INSIDE-NOISE).** RESULTS: recency_weighted/sgd100 Δelo −30.5±76 (INSIDE-NOISE — holds ≈ C) > lru/sgd100 −93.4±78 (signal — degrades below C) = lru/sgd300 −93.4±78 (identical 29.5/80 score; sgd=300 was sharper, 41 vs 31 decisive, but no net strength). **THREE FINDINGS:** (1) method delivers tight CIs as designed; (2) CURATOR MATTERS — recency preserves strength, lru forgets (~63-elo gap), the first concrete curated-buffer result; (3) more SGD on a fixed curated slice buys nothing. CAVEAT: all 3 degrade converged C (expected — 40-ep replay on a SUBSET of C's own buffer); the bake-off measured "which curator degrades least," recency won. decision: reject-as-C-improvement / PROMOTE the method + curator finding. **Reviewer APPROVE** (numbers match results.json; claims honestly hedged; caught+fixed a transposed wandb id↔recipe mapping). **TOP NEXT (high value):** a HEADROOM parent — fork off a mid-climb checkpoint (or deliberately-weakened C) where 40 epochs has real elo to GAIN, so curators compete on upside not just forgetting-resistance (none survive WL5 keep-last-n=3 → re-generate an early ckpt or use another run's). Then: direct recency-vs-lru head-to-head (tighter than both-vs-C); add curators (diversity, trouble-pinned) to the bake-off. consecutive_rejects: unchanged (research finding, not a knob reject).
- **RESUME STATE (2026-05-24, delta-e run-2 launched PROPERLY per gomoku-train skill — head-to-head, persistent paths, backfilled)**: Jason: "use the trainer skill and launch this properly, backfilling where necessary." Re-launched run-2 (the head-to-head Δelo run) with production hygiene instead of the ad-hoc /tmp launch: out-dir now `sweep_logs/delta-e-run2-20260524T044710Z/` (gitignored, persistent), archive moved off /tmp to `archives/wl5_replay_archive` (8.2G, same-volume mv — instant; joins archives/wl5_validation_v1.pt), `python -u` unbuffered, wandb on. **Backfilled the ephemeral run-1 artifacts** that the receipt pointed at: `/tmp/delo_run1/` → `sweep_logs/delta-e-run1/{results.json,harness.log}`, and updated the experiment-ledger artifact + command paths (/tmp → sweep_logs/archives). RUN-2 CONFIG: parent C = WL5 e10200, 3 recipes (lru/sgd100, recency_weighted/sgd100, lru/sgd300), 40-ep window, head-to-head 80 games/match (paired 4-ply openings), eval-sims 100, 6 parallel match workers. Each fork ~3 min train + ~15s VISIBLE parallel match (progress every 10 games); ~10 min total. pid 29082, driver log `sweep_logs/delta-e-run2-20260524T044710Z.driver.log`, completion watcher running → will deliver wandb URLs + ranked Δelo table. NOTE: this is a delta_e_harness research run, NOT a run_sweep cell — the title-card→ACK ritual is for production gomoku.train cells; Jason directed this launch. consecutive_rejects: unchanged.
- **RESUME STATE (2026-05-23, delta-e run-1 — Δelo flywheel runs end-to-end; anchor-ceiling found → head-to-head built; editable-install env hazard fixed)**: Jason ("yeah light it up, fan it out" / "wandb please") — ran the first real Δelo run (3 recipes off WL5 C, 40-ep window, anchored-eval 40 games/baseline). **Harness validated**: fork→replay-train→`latest.pt`→anchored-eval→Wilson-CI→implied-elo→Δelo→INSIDE-NOISE verdict→ranked table→results.json, 3 forks, each its own fresh wandb run (ms1pplps/91awvfib/jc228edo). **All 3 INSIDE-NOISE** — and the root cause is a METHOD-LIMIT, not equal recipes: the anchor ladder tops out ~1700 elo (strongest anchor lookahead:d4 @1500; C beats it 78%), so C and every fork pin near the ceiling and the difference drowns in the d4-winrate CI. No headroom parent survives either (keep-last-n=3 pruned all weak checkpoints; WL3.1 e1504 also ~1465-1567). **FIX BUILT: `--head-to-head` mode in delta_e_harness.py** — play fork directly vs C (model-vs-model via play_match_pickers+mcts_picker; relative Δelo=400·log10(p/(1-p)), Wilson CI through the logistic). Ceiling-free, max-sensitivity near 50%. Self-test + micro-smoke green (self-play→50%→Δelo+0). **TWO bugs fixed en route:** (1) train_replay `--resume` inherited the checkpoint's wandb_run_id → forks resumed WL5's run (o6cbjfnr); now an explicitly-named replay fork starts fresh. (2) **Editable-install path-leak (repo-wide hazard):** `__editable__.gomoku` finder MAPPING pointed `gomoku`→`.claude/worktrees/agent-a7d0c83fc186e0497/gomoku` (stale, cc6aa4e, 146 commits behind, missing `fuse_model_for_inference`). `python -m` dodges it (cwd first) but `python scripts/*.py` imported the STALE package — so run-1's eval ran on 146-commit-old gomoku (ceiling verdict robust; speed/exact-numbers not). Fixed via `pip install -e .` from main → finder repointed to main. NOTE: any agent running `pip install -e .` from its worktree re-clobbers this global finder — structural fragility in the worktree-agent spawn flow (flag for a guardrail). **IN FLIGHT:** head-to-head full-path smoke (1 fork, 3 ep, 12-game match) running (pid 20571); on green → run-2 = same 3 recipes off WL5 C head-to-head, wandb on. delta_e_harness.py change staged-not-committed (commit after run-2 confirms the h2h path end-to-end + spawn Reviewer). consecutive_rejects: unchanged (run-1 is research/needs_repeat, not a knob reject).
- **RESUME STATE (2026-05-23, LF1-followups fanned out — runaway knee mapped, tile-cap validated, instrument fixed)**: Jason: "proceed, fan out background subagents." Ran the LF1-followups block as a two-queue fan-out (4 worktree-isolated CPU agents + GPU sweep driven serially). **Outcomes, all coherent: (L2) runaway stability KNEE is in (384, 512]** — V=64/256/384 bounded, V=512 divergent (steps 22→154, wall 6.8→19.9s over 18 ep). **Method correction now in the skill:** divergence is in steps/wall/age/new-positions, NOT per-version tile (barrier-bounded ~85 by worker-min-games=64, V-invariant) — watching tile-only gives a false "no runaway" read; I also had to run the missing uncapped-V512-in-lab_train_cell control (I'd imported LF1's run_sweep divergence). **(L6) `--max-tile-games` cap VALIDATED end-to-end**: V=512+cap120 bounded (steps 53, wall 7.7s) vs uncapped divergent — closes the runaway loop; also shipped `--max-sgd-steps-per-epoch`, `--sgd-per-game` (all opt-in, WL5 byte-identical). **(L5) the runaway's extra SGD is REDUNDANT** (val/policy_ce best 3.9905 at cum-step 3.4k, flattens+reverses while steps 4.4×) → bounding the tile costs ~0 elo + improves elo-per-wall; L5+L6 compose. **(L1) warm-buffer/shrunk-buffer measurement mode** shipped (post-fill slope + BOUNDED/DIVERGING verdict; refuses cold numbers). **(L4) wall-clock-to-elo design doc** (`wall-clock-to-elo-metric.md`: MTTE primary/EPWH secondary, charter R-ELO-* diff [Class-B, un-applied], delta_e_harness.py gap analysis). 3 branches merged --no-ff; integrated tree verified (WL5 byte-identical, 7 flags coexist, tests green). **Reviewer: APPROVE** (headline reproduced from raw TSVs, attribution sound, opt-in gate holds, 20 tests green, charter Class-B left un-applied). Reviewer surfaced that the uncapped V=512 run reached the FULL 1.5M buffer at e15 and kept diverging — so divergence extends THROUGH buffer-fill, largely closing the pre-fill caveat; the capped-stays-bounded-post-fill piece is now CONFIRMED: a 32-epoch capped V=512 run reached the full 1.5M buffer at e28 and stayed BOUNDED (postfill steps_slope 0.019, verdict BOUNDED). Caveat closed — cap validated at steady state. **Top follow-ups:** (a) warm-buffer POST-FILL confirmation run (V=512 ± cap, shrunk buffer ~20k, ≥20 post-fill epochs) — closes the pre-fill caveat (all GPU runs were ~64% fill) AND exercises L1's mode on a real runaway; (b) optional V=448 to sharpen the knee; (c) Class-B charter Success-metric edit (L4 R-ELO-* diff) is the user's call. KEY UNLOCK: lab_train_cell CAN reproduce the runaway (previously thought run_sweep-only), so the harness can now study it. consecutive_rejects: unchanged (lanes promote/research, not rejects).
- **RESUME STATE (2026-05-23, LA1 — lookahead-eval perf pass, PROMOTE; new R-EVAL-* family opened)**: Jason redirected ("perf pass on lookahead eval"). Target = `lookahead_player`, the alpha-beta **Elo anchor** used by eval_worker + the training-loop eval (the known-slow path — train.py:341 dropped depth=2 as default at "45s+"). **Result: ~6.3× faster (depth=4 145.6→22.94 ms/move; depth=2 6.5×), byte-identical move selection.** Native state_ops were already C-fast; the bottleneck was 3 pure-numpy per-node helpers in baselines.py — vectorized `_find_immediate_wins` (dense per-cell max), `_candidate_moves` (precomputed (81,81) `_NEIGHBOR_MASK` gather), and added candidate-restricted `_score_cells`. Proven behavior-identical across 360 positions → TQ gate satisfied by construction (anchor outputs unchanged). Merged to main `--no-ff` (commit on feat/perf-lookahead-eval). New surfaces: R-EVAL-LA in best-cells.md + baselines.md; `scripts/bench_lookahead.py` is the reproducible harness. **Reviewer: APPROVE** (integrity/math/equivalence-edge-cases/surfaces all verified; concurrent 6d47bbb work intact). Reviewer non-gating flag actioned → committed `tests/test_baselines_vectorized_equiv.py` (helpers vs brute-force refs). The LF1-followups block below remains the standing TOP direction for the GPU queue (metric-validity work); LA1 was a self-contained CPU-queue eval-path lane. Follow-up levers for lookahead are diminishing-returns (history-free apply ~8-10% but shared with MCTS; numba/cython = Class C) — NOT queued unless eval cost resurfaces. consecutive_rejects: unchanged (LA1 is a promote).
