...
confidence: HIGH for the boundary + cap mechanism (stark 4-run contrast, same recipe). CAVEAT (UPDATED by Reviewer): the uncapped V=512 run actually FILLED the 1.5M buffer at epoch 15 and kept diverging (steps 149→154 through e15→e18) — so the divergence already extends THROUGH buffer-fill, not just pre-fill. The bounded runs (V=256/384/capped) are at ~64% fill (lower inflow). Only remaining open piece: confirm the CAPPED run stays bounded once its slower-filling buffer fills (~e28) — a cheap bounded run.
