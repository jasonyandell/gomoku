...
result: |
  Gumbel@100 generator dominates: gumbel-fast5s 1620 ✓ / gumbel-wave 1580 ✓ both ≫ sims100 control (697, plain MCTS@100) → Gumbel RESCUES cheap sims; both ≫ open-div4 (776, v1 ceiling champion) on equal-or-less budget. forced (KataGo) 1262. Fixed-step (fast5s) vs wave (gumbel): CO-EQUAL — peaks within eval noise (1620 vs 1580); on the canonical Δelo/hr (standings.md, (peak−389)/wall-to-peak) wave EDGES it 3031 vs 2825 (~10%; flips to ~1.5× for fast5s on a time-to-peak basis). NO clean rate separation. Fixed-step's wins are STRUCTURAL: peak in one contiguous slice + structurally can't run away (vs sgd_per_position) + validated the gen-flooding fix → the cleaner v4 control. Healthy: reuse ~1.4 settling ~1.05, pl 4.39→1.72 descending vs climbing cumsteps (productive, not redundant). Regime: gen≈0.3s ≪ train≈2.6s (generation FLOODS the trainer; "gen-bound" stale).
...
artifacts: sweep_logs/delta-e-run2-20260524T044710Z/{results.json, .driver.log, fork_*/trainer.log}; wandb (fresh, verified vs per-fork trainer.log): recency_weighted_sgd100=6g8gb5m9, lru_sgd100=7g12xjyf, lru_sgd300=w6n4n0g8.
...
reviewer: APPROVE — all Δelo/CI/W-L-D/verdicts match results.json exactly; "~3× tighter" fair (per-recipe 2.8/3.0/3.0); "recency>>lru" honestly hedged (marginal CIs overlap, robust read = the qualitative INSIDE-NOISE-vs-signal split); sgd=300==sgd=100 is a real 0.5-draw-scoring coincidence not a bug; TQ gate correctly does not fire (nothing promoted, best-cells.md untouched). Caught + FIXED a non-blocking nit: the wandb id↔recipe mapping was transposed (now recency=6g8gb5m9, lru/sgd100=7g12xjyf, verified vs trainer.log).
...
artifacts: sweep_logs/delta-e-run1/results.json, sweep_logs/delta-e-run1/harness.log (backfilled from /tmp to the persistent repo path 2026-05-23); wandb runs (fresh, NOT WL5's): delta-e-lru_sgd100=ms1pplps, recency_weighted_sgd100=91awvfib, lru_sgd300=jc228edo (project jasonyandell-forge42/gomoku). Archive A now lives at archives/wl5_replay_archive (moved off /tmp).
commands_run: python scripts/delta_e_harness.py --parent <C> --archive-path archives/wl5_replay_archive --window-epochs 40 --recipe 'lru:sgd_steps_per_epoch=100' --recipe 'recency_weighted:sgd_steps_per_epoch=100' --recipe 'lru:sgd_steps_per_epoch=300' --eval-games 40 --eval-sims 100 --wandb --out-dir sweep_logs/delta-e-run1
decision: needs_repeat
reviewer: APPROVE — receipt numbers reconcile exactly with /tmp/delo_run1 artifacts; head-to-head math sound (elo_from_score_rate + Wilson-CI-through-logistic); paired-opening color-balance + winner-attribution verified line-for-line vs play_match_pickers; needs_repeat correctly skips the TQ promotion gate; editable-install env fix coherent and run-1's ceiling verdict robust to it. Non-blocking nits: vestigial archive_path/baselines in h2h results.json; silent floor on odd --eval-games; dead sims/c_puct params in head_to_head_eval.
