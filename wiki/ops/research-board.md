...
> **🔬 MECHANISM-OF-THE-CEILING probe (researcher, 2026-05-28, read-only) — the ~50% ceiling is SEARCH-DEPTH, not structural; "structural" was premature.** Subagent code-walk of the lookahead4 opponent + our eval harness against three hypotheses (theory-drawn / search-depth / opening-leak):
> - **Smoking gun:** in `derby-v7-mate-discount/checkpoints/eval_results.jsonl` across 2000+ epochs, **black LOSSES are essentially ZERO** — only the win/draw split moves (draw rate slides 70%→30% with maturity, losses stay 0). That's the signature of a tactical-floor opponent the net can DEFEND against but can't OUT-SEARCH — NOT a stochastic noise process and NOT a theory ceiling (those would either pin black-win-rate at one number or scatter losses).
> - **Mechanism:** lookahead4 is depth-4 alpha-beta with branching capped at top-12 (`baselines.py:430-507`) + a depth-0 quiescence step that blocks immediate 5-in-a-row threats + threat-aware static eval at leaves → it effectively sees the full 4-ply tactical horizon. Our net's eval-side search uses **only 100 MCTS sims** (`eval_worker.py:66-68`, `eval.py:61`), `add_root_noise=False` (`eval.py:72`). With 12-candidate root branching, the net gets ~8 sims on the relevant subtree — **too shallow to find >4-ply forcing wins**. A winning line that exists at 5–6+ plies is invisible at 100 sims, so the net settles for the draw.
> - **Consequence — the 100% target may be reachable with the EXISTING champion, NO training change, just by raising eval sims for hard baselines.** Cheap one-shot probe: re-eval one strong v8 checkpoint against `lookahead:depth=4` only, 40 games × {100, 200, 400, 800} sims, **black-side only**. If win-rate climbs from 51% into the 70–90% range, H2 is confirmed → the cockpit lever is **"eval-sims for hard baselines"** (in-loop n=20/sims=100 is just under-resourced for depth-4) and the 100% objective is essentially solved by an eval-config tweak. If win-rate is FLAT across sims, demote to H1 (theory ceiling) and redirect.
> - **This REFRAMES `derby-9q4` (draw-contempt).** Draw-contempt is a TRAINING-side fix for what may actually be an EVAL-RESOURCE problem. If the sims-probe shows H2 is right, draw-contempt is solving a non-problem (or worse — risks white-leaks for a search-bound gap that more eval sims would close cleanly without any training risk). **Run the sims-probe FIRST**; gate draw-contempt on its outcome. Secondary lever if sims-scaling plateaus <100%: toggle small root-Dirichlet noise at eval time (currently OFF) so the net doesn't collapse onto its top prior under tactical pressure.
> - **Note: this is a SUBMISSION (read-only finding + suggested probe), not a ranking decision.** Whether to run the probe, whether to raise the in-loop eval sim count (changing the 100% target's definition), and whether to gate `derby-9q4` on the probe outcome are the ranking owner's calls. The researcher delivers the mechanism + the experimental design.

> **📚 PAPER-MINED CANDIDATES post-H2 (researcher survey, 2026-05-28; user prompt "find techniques from papers… we can get this to at least 100%").** Fanned-out subagent mined AlphaZero/MuZero/KataGo + recent arxiv against the full kill-list. Three actions filed; ordered by EV given the H2 search-depth finding:
> - **#1 — Reanalyze (KataGo §4; MuZero appendix G).** Periodically re-search OLD buffer positions with the CURRENT (stronger) net + a DEEPER MCTS budget (e.g. 400 vs gen's 100), overwrite the stored `(pi, z)` in place. Directly attacks H2 — bakes deeper-search insights into the training targets so the net's 100-sim eval prior absorbs the deeper plan; compounds with `--value-discount 0.98` and the VCF value-stamp (sharper signal on the same data). KataGo credits reanalyze with ~2× elo/compute. **Action: epic `derby-3vs` already filed with 3 sub-issues labelled DEFERRED; promoted `derby-fm9` (engine, 1/3) → OPEN (un-deferred) with H2 motivation note; promote 2/3 (`derby-1nt` scheduler) and 3/3 (`derby-wdw` cell) sequentially after 1/3 lands.** Risk: single-GPU starvation (KataGo uses a separate worker pool) — the scheduler issue's starvation guard handles this; under-load 50+ epoch re-race is the gate (the ingest-flooding lesson).
> - **#2 — Eval-time root VCF overlay (eval-ONLY, no gen, no training).** At each EVAL move, run a bounded `solve_vcf` from the root first; if it returns a PROVEN win, play it; else MCTS choice unchanged. The most surgical attack on the H2 lookahead4-black gap — VCF deterministically finds forced-fours, the same tactical class lookahead4 defends. Reuses the already-bounded solver from `derby-b6r`. ZERO gen-hot-path cost (the property in-search-VCF lost on at gen-time). **Action: filed `derby-ehw` as a code-only `derby-idea` issue — no derby cell (eval-config, not a training cell); ranking owner runs the existing champion with `--eval-vcf-nodes {0,200,800,3200}` × eval sims and measures distance-to-100%.** Pairs naturally with the H2 eval-sims probe — both are eval-config interventions on the EXISTING champion. Highest probability-of-success of any candidate. Risk: if the draws are non-VCF (need VCT-class threes), only partially closes the gap.
> - **#3 — SWA (Izmailov 2018, "Averaging Weights Leads to Wider Optima"): post-training average of the last K champion peak checkpoints**, eval the averaged checkpoint vs champion. Distinct from our `--ema-tau 0.99` (which is an EMA *during* training, used by gen workers). We have no BatchNorm so no recalibration needed — it's literally `scripts/swa_average.py` + one H2H. Cheapest possible free-elo check. **Action: NOT filing an issue yet** to avoid piling on a dead runner; recommend the ranking owner run this on the matured champion's `_peaks/` as a 10-minute side-experiment when the runner resumes. Expected magnitude small (10–30 elo at supervised scale; possibly less in RL).
> - **Conditional follow-ons (gated, NOT filed):** **HL-Gauss distributional value head** (Farebrother 2024, "Stop Regressing") = WDL-but-finer (51 bins, Gaussian-smoothed targets) — paper specifically argues for drawish/coin-flip regimes (our 60–70%-draw zone); race only if reanalyze underperforms (avoid WDL's "+56 but never beats champion" repeat at higher engineering cost). **Opening-book seeded self-play** (champion's own top moves at sims=1000 as opening seeds) — distribution-shifts training onto the binding lookahead4-black-draw branches; lower EV than reanalyze; defer.
> - **KILLED in this survey (one-line each):** SAM (2× compute step = trainer gen-flood tax, wrong axis); opp-reply head standalone (correlated-killed with v4/v5 `signal`, competes with VCF stamp on the same decisive-lines axis where VCF won); fork-augmentation in training (strictly dominated by reanalyze — same maturity replays vs current-net replays); per-action Q-WDL head (gated on dead WDL); KL-policy-distillation from VCT teacher in eval (covered by #2 more directly); population-based training (wrong scale — one GPU).
> - **Net effect on the queue (post-survey):** `derby-fm9` OPEN (reanalyze engine), `derby-ehw` OPEN (eval-VCF overlay), `derby-9q4` OPEN (draw-contempt, gated on H2 sims-probe), `derby-x-medium-signal` landed-but-stale (the net-capacity coupling lost; medium-signal needed only if reanalyze underperforms). All ready for the bead-runner (issue dispatcher) / GPU runner whenever the lab resumes.

> **🎯 RESUME PLAYBOOK (researcher cockpit, 2026-05-28) — recommended experiment order the moment the GPU runner resuscitates.** The bead-runner shipped the full H2-motivated code surface in ~20 min (derby-9q4 draw-contempt cell, derby-fm9+1nt+wdw reanalyze 1/3+2/3+3/3, derby-ehw eval-VCF overlay) while the GPU runner has been down since `02:00:43Z`. Ordered by cost↑ × probability-of-success↓ (cheap+likely first; gate the rest on outcomes):
> 1. **(eval-only, ~CPU/MPS minutes) Run the H2 eval-sims probe + eval-VCF overlay together on the matured champion** (`sweep_runs/derby-v7-mate-discount/_peaks/mate-discount/peak.pt`) vs `lookahead:depth=4` only, 40 games/cell, BLACK-side: vary `--sims {100,200,400,800}` × `--eval-vcf-nodes {0,200,800}` = 12 cells. Read distance-to-100% via `scripts/report_100pct.py`. If any cell hits ≥80% black-wins, the 100% target is essentially solved by **eval-config alone** (zero training change) — supersedes everything below. *This is the cheapest possible decisive test.*
> 2. **(GPU, one peaks-averaging + one eval) SWA on the champion's `_peaks/`** — `scripts/swa_average.py --k 10 --stride 5`, eval the SWA checkpoint H2H vs champion. ~10-min side-experiment; no BatchNorm so no recalibration. Free elo check (10–30 elo plausible).
> 3. **(GPU, derby cell) Race `derby-x-reanalyze` (KataGo §4)** as the next training-side lane. Highest-EV training-side lever — bakes deeper-search insights into stored `(pi,z)` targets so the net's eval prior absorbs the deeper plan. Watch for trainer starvation under single-GPU (the scheduler's cooldown-feedback guard handles this; under-load epoch-50+ re-race is the gate per the ingest-flooding lesson).
> 4. **(GPU, derby cell, GATED on H2 sims-probe outcome) Race `derby-x-draw-contempt`** — training-side fix surgically aimed at lookahead4-black draws. If step 1 closes the gap by eval-config alone, **don't race draw-contempt** (it's a non-problem at that point, and risks white-leaks). If step 1 doesn't close it, draw-contempt is the next training-side bet.
> 5. **(GPU, derby cell) Race `derby-x-hlgauss` (Farebrother 2024 "Stop Regressing")** — HL-Gauss distributional value head (51-bin categorical with Gaussian-smoothed target). Now filed as `derby-tn4` (UN-GATED per "keep researching" 2026-05-28). Extends WDL's classification-not-regression win (+56 H2H, the lone keeper) with resolution — distinguishes "looks 0.05 better" from "looks 0.15 better" where WDL's 3 bins collapsed both into draw. Paper specifically argues for drawish regimes (our 60-70%-draw zone). Training-side, zero gen tax. Race in PARALLEL with reanalyze (independent levers; both lose if both lose, both can win). Risk: could reproduce WDL's "won big but never beat champion" pattern — if it matches WDL, retire and move on.
> 6. **(GATED, low-priority) Remaining conditional follow-ons** if all of 1–5 underperform: opening-book seeded self-play (defer); `derby-x-medium-signal` (only if reanalyze underperforms AND scale axis re-opens).

> **🔬 THREE MISSING SEARCH-SIDE SEAMS (sub-agent code-walk 2026-05-28, biggest single finding of the session).** Reading the actual MCTS code surfaced three places where our search lacks **standard AlphaZero/KataGo practice** — all eval-only fixes, all stack with each other AND the existing eval-VCF overlay / sims-probe / FPU sweep, all attack the H2 search-depth ceiling. None of these is on the kill-list because none was previously surveyed.
> - **(A) FPU reduction at eval — `gomoku/mcts.py:71` sets Q for unvisited children to LITERALLY 0.0** (not even AGZ's "inherit parent V" fallback). In drawish positions (the binding lookahead4-black-draw regime), `Q=0` makes unvisited siblings look IDENTICAL to weakly-explored ones → UCT chases siblings via the U term even when the principal variation is the only line with a forcing win → ~30–50% of the 100-sim budget wasted re-confirming dead-equal siblings. KataGo uses `c_fpu ≈ 0.45` (subtree) / `0.20` (root); LCZero default 0.33; standard since AGZ 2017. **Filed as `derby-3w0`** (~5 LOC change, eval-only, default OFF = byte-identical).
> - **(B) No tree reuse across plies at eval — `gomoku/eval.py:124` `mcts_picker` builds a fresh `MCTSGame` *inside* `pick()`** for every move, throwing away the entire previously-explored subtree. Original AlphaZero paper reuses; KataGo `cleanupBeforeSearch`; LCZero `--reuse-tree=true` default. In a 6-ply forcing sequence the search at ply N already explored ply N+2; we re-search from scratch. Tree reuse effectively multiplies the sim budget by the reuse-fraction WITHOUT spending more compute — cheapest possible "more sims" without raising sims. **Queued for next tick's issue** (~30 LOC + signature change to make picker stateful; `mcts.py:228` already has `advance_root` — just need to wire it through `play_match_pickers`).
> - **(C) No proven-win/loss propagation in MCTS — only `is_terminal` (5-in-row already reached) is recognized.** A forced 3-ply win that requires ONE more sim to materialize is invisible to root selection. KataGo "graphSearch" §3 + AGZ Appendix propagate `proven_value ∈ {-1, +1}`: parent with a child of `proven=-1` becomes `proven=+1` (child loses → parent wins); parent where ALL children are `proven=+1` becomes `proven=-1`. Mechanistically distinct from `derby-ehw` (eval-VCF root overlay) — that one solves only at the ROOT, this propagates at every interior node where it might fire. **Queued for the tick after `derby-3w0`** (~80 LOC, heaviest of the three but still eval-only and byte-identical-off).
> - **Net effect when all three land + the existing eval-VCF overlay + sims sweep:** an 8-cell (or larger) eval-config matrix on the EXISTING champion — `{sims} × {eval-VCF} × {FPU-c} × {tree-reuse on/off} × {proven-prop on/off}` — that maximally probes whether the 100% target is reachable with ZERO training change. If all of A+B+C+eval-VCF compound, this could close the lookahead4-black gap entirely on the existing matured champion. If none of them help, then HL-Gauss + reanalyze are the right training-side fallbacks — but only then. **Training-side recipe space is exhausted for this objective; the next real wins are in the search engine itself.**

> **🔎 Search-contempt (focused-mine of 2024-2026 papers, 2026-05-28).** Singh & Eindhoven, "Search-Contempt" (arxiv [2504.07757](https://arxiv.org/abs/2504.07757), April 2025): a self-play POSITION-DISTRIBUTION lever — at each self-play decision, with prob `--contempt-p`, perturb root selection to favor moves whose child Q is near 0 (most contested), recording those positions as normal training data. Result: training distribution oversamples hard-to-convert positions. Distinct from EVERY filed lever (NOT draw-contempt's value-target reshape, NOT VCT-teacher's solver distillation, NOT playout-cap's target-pruning, NOT buffer-curation's sampling lever, NOT opening-book's external openings, NOT any eval-side knob). Demonstrated in Odds Chess (asymmetric/hard-to-convert starts) — closest published analogue to our lookahead4-black draw plateau. **Filed as `derby-qoq`** (champion clone + `--contempt-p 0.5` on worker, byte-identical when off). **GATED on the H2 sims-probe outcome** — if eval-config alone closes the gap, don't race; otherwise the next training-side parallel arm alongside draw-contempt / reanalyze / HL-Gauss. Class-B risk: single-paper / single-domain at submission time; position-distribution drift could regress value-head on standard positions if p is too high (color-split eval catches it).

> **🧠 FRAME-LEVEL SYNTHESIS — closed-self-play-loop covariate shift (Jason teaching question, 2026-05-28).** Why does the matured champion need ~2848 epochs / ~22h of self-play to reach 100% on a "dumb" heuristic-Bwin (and why are most non-matured lanes nowhere near it: v9-small 57%, v9-medium 29%)? The deep reason isn't "AlphaZero isn't strong enough"; it's **structural to the algorithm**, not a defect of our lab. Three compounding causes:
> 1. **Self-play is a closed loop with covariate shift.** The net only ever trains on positions IT ITSELF would play. The heuristic plays *different* moves (locally weaker, sometimes weird) → creates positions that NEVER appear in our training buffer. The net's policy/value is well-calibrated on the "optimal-vs-optimal" manifold and **OOD on heuristic-induced positions**. It has to *generalize* from nearby positions — works sometimes, fails unpredictably (the fragility).
> 2. **AlphaZero learns Nash, not exploit.** Self-play converges (asymptotically) to a Nash-equilibrium strategy — best response to a player who plays like you. Against a sub-optimal opponent, the *exploitatively optimal* strategy differs (it targets the opponent's specific weaknesses). Self-play can't learn the exploit because the exploit-target (heuristic's specific mistakes) is never in the training distribution.
> 3. **Sample complexity vs eval search budget.** Original AZ used millions of games + thousands of eval sims. We use thousands of games + 100 eval sims. Asymptotic guarantees become brittle here: a slightly-miscalibrated prior on an OOD heuristic-induced position needs more sims to recover, and 100 isn't always enough.
>
> **This frame explains the entire lever taxonomy in one unified picture:**
> - **Training-side levers (reanalyze, search-contempt, opening-book, draw-contempt, HL-Gauss)** all try to *break the closed self-play loop* — get more diverse positions into the buffer (reanalyze: sharper targets via deeper search on existing; search-contempt: bias toward contested positions; opening-book: external openings inject OOD starts; draw-contempt: change what "winning" means so policy stops settling for self-play-equilibrium draws; HL-Gauss: finer value-resolution on the contested middle).
> - **Eval-side levers (FPU, tree-reuse, proven-prop, eval-VCF, more sims)** all try to *make the limited search competent enough to recover from a slightly-miscalibrated prior at runtime*. They don't fix the training distribution; they let the eval search fight harder against a not-quite-right policy.
>
> Either path can close the gap; our current investment leans heavily eval-side because it's cheaper (no training time, no training risk, byte-identical-off, all stack) — but the *reason* eval-side helps at all is the same covariate-shift problem; we're compensating at the other end of the pipeline.
>
> **If eval-side underperforms, the highest-leverage training-side move is the AGZ-Master-style FIX** that AlphaZero proudly removed and KataGo partially put back: mix some games against EXTERNAL bots (here: heuristic / lookahead2 / lookahead4) directly into the training buffer. This is more direct than opening-book seeding (which only injects OOD openings; the rest plays out via self-play). Heuristic-mix injects the *full* heuristic-induced game distribution. Design care needed: only the NET's move decisions should generate policy targets; the heuristic's moves are trajectory-only. Currently a **DEFERRED design — not filed** until eval-side investment is judged; if filed it would be a new derby-idea (medium implementation lift, distinct from all current levers).
>
> **Deeper takeaway (the student-answer):** AlphaZero is "better" in the asymptotic sense, against opponents from a similar distribution. Against an OOD opponent at finite training budget, the situation is more interesting — and "more training" alone doesn't close the gap, because more self-play just tightens the closed loop, not broadens it. That's a structural property of the algorithm, not a bug in our setup. The matured champion *did* get to 100% heuristic-Bwin eventually (at 80-game sample), but it took ~22h partly because every cycle of self-play it had to re-learn its own mistakes against itself — it never got positive examples of "this is what happens when a weak opponent does X."
>
...
> - **🎯 THE NEW CEILING — lookahead:depth=6 (40g confirmed).** **Baseline: 14W/6L/0D black + 0W/4L/16D white = 70% Bwin, 20% Wloss, dist 0.5** at default config (sims=100, no levers). Unlike la4 (always wins or draws, never loses), la6 beats the champion in REAL games — 6 black losses + 4 white losses across 40 games. This is the genuine ceiling worth attacking.
> - **🏆 LA6 4-AXIS MATRIX VERDICT (16 cells, 40g/cell, completed 2026-05-28 20:10Z) — THE EVAL-SIDE INVESTMENT WORKS, AT THE RIGHT OPPONENT TIER.** Tested vcf × fpu × reuse × proven_prop on champion vs la6:
>
>     | rank | config | Bw/L/D | Ww/L/D | Bwin | Wloss | dist |
>     |---|---|---|---|---|---|---|
>     | 🥇 1 | fpu=0.45 + reuse=1 + pp=1 (vcf=0) | 19/0/1 | 0/0/20 | **95%** | **0%** | **0.050** |
>     | 🥇 2 | fpu=0.45 + reuse=1 + pp=1 + vcf=800 | 19/0/1 | 0/0/20 | 95% | 0% | 0.050 |
>     | 3-6 | fpu=0.45 alone (reuse=0, pp/vcf any) | 16/0/4 | 0/0/20 | 80% | 0% | 0.200 |
>     | 7-8 | fpu=0.45 + reuse=1 (pp=0) | 15/0/5 | 0/0/20 | 75% | 0% | 0.250 |
>     | 9-12 | vcf=800 + fpu=0 (any reuse/pp) | 18/2/0 | 0/7/13 | 90% | 35% | 0.450 |
>     | 13-14 | reuse=1 + fpu=0 + vcf=0 | 15/5/0 | 0/4/16 | 75% | 20% | 0.450 |
>     | 15-16 | baseline (all OFF) | 14/6/0 | 0/4/16 | 70% | 20% | 0.500 |
>
> **Per-lever verdict (vs la6, EVAL-ONLY, no training change):**
> - **FPU reduction (`derby-3w0`): WINNER, THE dominant lever.** Every top-8 cell has FPU on. THE WHITE-SIDE SAVIOR: every fpu=0.45 cell has **0% Wloss** (vs 20-35% without). FPU alone goes 70%→80% Bwin and 20%→0% Wloss.
> - **Tree-reuse (`derby-jmi`): HELPS WHEN PAIRED WITH FPU.** 80%→95% Bwin when added to FPU. Alone (fpu=0): marginal Bwin gain, no white-side help.
> - **Proven-prop (`derby-b3n`): THIRD LEG.** Only matters with fpu=0.45 + reuse=1 (75%→95%). Otherwise neutral.
...
> 2. The eval-VCF overlay (`derby-ehw`) is **demoted to "neutral or harmful"** for the la6 objective; FPU subsumes it. Still might apply in other regimes (different opponent, different game).
> 3. **The pivot from "training-side levers (HL-Gauss, draw-contempt, search-contempt, reanalyze)" to eval-side levers is doubly validated** — the cheapest, lowest-risk path closes the gap; training-side levers are now SECONDARY and only worth racing if a HARDER objective (la8? external strong engines?) needs them.
> 4. The new objective should be: find the NEXT ceiling. Either lookahead:depth=8 (if it's even slower-but-tractable) or external strong engines (Rapfi, etc.). The eval-side levers will likely transfer.
...
**The lab runs as a single GPU executor (the derby) + code-only issues (other sessions).**
...
never swapped; everything gets run). **Issues never run the GPU** — an issue = code-only
work for another session that lands a cell in `run_sweep.CELLS` "available for the
derby." Config-only levers (existing flags) skip issues — the runner just adds the cell
and races it. (Task tracking is GitHub issues as of 2026-05-28. Full model:
`wiki/topics/research-loop.md`; memory `project_derby_operating_model.md` is the resume index.)
...
- **Backlog (GitHub issues, `derby-` prefix in titles, label `derby-idea`):** un-gated
  ideas carry the `deferred` label (excluded from the ready query); gate =
  `gh issue edit <N> --remove-label deferred`. Code-heavy epics: VCT solver
...
*(Loop operational note, for future researcher-loop sessions: task tracking is GitHub issues
as of 2026-05-28 — one remote, visible everywhere. The old beads-era merge gotcha — `bd
create`/`bd update` staging a dirty `.beads/issues.jsonl` on `main` that blocked a `feat→main`
merge — is GONE; there's no local task-store file to commit before merging.)*
...
- v5 stack the winners, v5 FINAL verdict
- v6 researcher round 1, v6 FINAL verdict
- Title cards (C0-baseline, sims-400, sims-100, sgd-800, buf-30k, open-div4, ema-099, temp-16, v3-gumbel, v3-playoutcap, v3-forcedplayout, v3-swa, v3-auxhead)
- v1 Standings (auto-updated 2026-05-24T15:31:05Z)

For the **current champion (v9 CONCLUDED)** and **live open candidates**, see above.
