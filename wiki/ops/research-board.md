...
> 5. **(GATED, low-priority) Conditional follow-ons** if all of 1–4 underperform: HL-Gauss distributional value head (file new bead — race only if reanalyze fails), opening-book seeded self-play (defer), `derby-x-medium-signal` (only if reanalyze underperforms AND scale axis re-opens).
>
