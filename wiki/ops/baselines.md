...
| 2026-05-23 | `7c26506` main | M5 Max / MPS (trainer) + ANE/GPU/Mix (workers via Core ML routings) / idle | L09e 2-cell routing diagnostic at L09 reference shape; small / W=8 / G=8 / S=400 / V=64 / EMA τ=0.99 / grad_accum=4 / `--evaluator coreml`; CPU_AND_GPU and ALL routings; 30s warmup + 120s measure / 11 epochs each | epochs/sec, games/sec, aug pos/s, trainer_step_s_p50 | **CPU_AND_GPU = 1,908.3 aug/s; ALL = 1,989.8 aug/s; vs L09 CPU_AND_NE = 1,930.3 aug/s** (REJECT — across-routing spread 4.3%; L09 reject is final, not a routing artifact). All three routings still ~40% below R-TRAIN-WL5 (3,297.6); compute-units axis cannot rescue ANE at small/V=64. trainer_step_s_p50 across routings: 0.0197-0.0227s (all ~half of R-TRAIN-WL5's 0.0512s — MPS-relief real in all 3). | `sweep_logs/lab-L09e-{cpugpu,all}-20260523T162*Z/` |

## Engine-Isolation / Residency References

> **2026-05-23 reconciliation (important):** the two 2026-05-22 `coreml_ane_residency_scout.py` rows below show the gomoku model **ANE-resident** (4,061 mW ANE rail at b32; 122k pos/s) under the scout's "fixed fused" export. But **L09e' (2026-05-23) shows the SAME model running on CPU/BNNS — NOT the ANE — under `lab_train_cell`'s `coreml_evaluator.export_model_to_coreml` path** (`convert_to="mlprogram"`). **Conclusion: our model CAN run on the ANE; our production export path doesn't achieve residency.** The two export paths differ (the scout builds a fixed-fused model; coreml_evaluator uses the mlprogram converter which may emit ANE-hostile ND ops per hollance/neural-engine). This is the load-bearing motivation for L09i (mlpackage op inspection): diff the two export paths' ops to find what forces CPU fallback. See [m5-max-cross-engine-coupling.md](../topics/m5-max-cross-engine-coupling.md) Part 1.

| Date | Worktree | Hardware | Command shape | Metric | Result | Artifact |
| --- | --- | --- | --- | --- | --- | --- |
| 2026-05-23 | main | M5 Max / MPS (trainer) + CPU-BNNS (workers) | L09e' residency proof: `lab_train_cell` tiny/W16/V64/coreml CPU_AND_NE + `sample <worker_pid>` | engine attribution (no sudo) | **Workers on CPU/BNNS, NOT ANE**: hot path `E5RT::Ops::BnnsCpuInferenceOperation::ExecuteSync`; no `H11ANEServicesThread`; system GPU monitor ANE 0%. Cell replicated L09c at 10,431.6 aug/s. Cross-check CPU_AND_GPU = 10,202 aug/s, still BNNS-CPU. **L09c cap pinned at `coreml-isolated`, not `ane-metered`.** | `sweep_logs/lab-L09e-prime-20260523T181123Z/{summary.tsv,sample_w0_T76s.txt}`, `sweep_logs/lab-L09c-cpugpu-20260523T182038Z/` |
| 2026-05-23 | main | M5 Max / CPU workers + GPU hog | Lpwr power-coupling: `lab_train_cell` tiny/W16/V64/coreml CPU_AND_NE ± `scripts/gpu_load_generator.py --matrix 8192` | worker aug/s with/without GPU saturation | **Clean cool-chip A/B: 10,431.6 → 1,905.2 aug/s (−81.7%)** when GPU saturated at ~11 TFLOP/s; trainer only +14% (asymmetry → shared package power envelope). Intensity sweep {0,2048,4096,8192} thermally confounded (non-monotonic; baseline drifted 10,431→8,531). Coupling real & large; mechanism not cleanly pinned (needs cold-chip interleaved A/B + powermetrics). decision=needs_repeat. | `sweep_logs/lab-L09g-prime-gpustress-20260523T182622Z/`, `sweep_logs/lab-Lpwr-gpu-coupling-20260523T183502Z/` |
