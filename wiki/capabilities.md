...
| **One-worktree-per-task workflow + janitor** (off `main` → `feat/<slug>` → `merge --no-ff` → push; reclaim crashed leaks) | `scripts/gh_worktree.py <N>`, `scripts/reclaim_worktrees.py --apply` | [topics/worktree-hygiene.md](topics/worktree-hygiene.md) |
