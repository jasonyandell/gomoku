...
> **Status: LIVE (2026-05-23 → 2026-05-24).** Settled measurement page — the
> mechanism is resolved (GPU working-set/occupancy, not FLOP count) and 3 of 4
...
- **Core ML runs our gomoku model on the CPU (BNNS), not the ANE** — confirmed by process sampling (L09e'). **CAVEAT (per [coreml-design-envelope-and-our-fit.md](coreml-design-envelope-and-our-fit.md) § "Current state"): this verdict was correct *for the lab export* but is no longer the whole story — a symbolic `ct.RangeDim` batch-dim export bug silently demoted the program to CPU/BNNS; the L09i-fix (fixed static batch) restored genuine ANE residency.** Despite `--coreml-compute-units CPU_AND_NE`, no `H11ANEServicesThread` spawns and the hot path is `E5RT::Ops::BnnsCpuInferenceOperation::ExecuteSync`. Under `CPU_AND_GPU` it *still* picks CPU. Core ML's cost model chooses CPU for our tiny/V=64 model regardless of the routing hint.
...
- **ANE-resident workers also throttle under GPU load, and the coupling is bidirectional.** A clean pure-self-play A/B (L09i-fix-load-v2, no trainer → no wave-barrier confound) shows ANE workers **−35%** under a GPU hog (3,548 → 2,307 aug/s) — the ANE is **not** contention-immune. Notably the coupling runs both ways: the 16 busy ANE-resident worker processes throttle the GPU hog from ~10.7 down to **~2.72 TFLOP/s** (~4×), so worker↔GPU package load browns out in both directions and settles at an equilibrium (workers −35%, hog −75%). **Package-load-attribution caveat (stated once here, referenced throughout this page):** the throttling force is *total worker package load* — Core ML eval **plus** CPU-side MCTS — **not the ANE engine specifically**; this cell can't isolate which. Because the hog intensities aren't matched (~2.72 here vs ~11 in Lpwr), the ANE reads gentler than the CPU/BNNS −82% but **the intensities differ — not a clean fragility ranking**. (This supersedes the earlier "generation held / positive lean" reading, which was a trainer-barrier artifact.) See `wiki/ops/experiment-ledger.md` 2026-05-23 "L09i-fix-load-v2".
...
- **The coupling is bidirectional.** The GPU hog reached only **~2.72 TFLOP/s** here, vs ~10.7 alongside a light trainer in fix-load — i.e. the 16 busy ANE-resident worker processes (total worker package load, per the package-load-attribution caveat in the TL;DR) throttle the GPU hog roughly **4×**. The busy workers and the GPU brown *each other* out through the shared package power budget and settle at an equilibrium (workers −35%, hog −75%).
...
- ~~Does the same throttle hit ANE workers (if we ever get the model ANE-resident)?~~ **Answered (L09i-fix-load-v2): yes — ANE workers throttle −35% under a GPU hog in clean pure self-play, and the busy worker processes (total package load, per the TL;DR caveat) throttle the hog ~4× back (bidirectional package-power coupling). The ANE is not contention-immune. See Part 2.** (Open follow-up: where the workers↔GPU equilibrium lands under a *real* trainer's GPU load vs the synthetic hog.)
