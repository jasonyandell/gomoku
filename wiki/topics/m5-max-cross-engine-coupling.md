...
### Rapfi-pool underutilization is a READ-PATH bug (per-line thread spawn + GIL), NOT stdio latency or compute (2026-06-24)

A warm pool of **60 Rapfi engines** driving the gomocup stdin/stdout protocol (15×15 NNUE, `--mine-bfs` BFS teacher-mining, `max_node=20000`, `max_pv=8`, pure-CPU — no GPU/MPS) lands at only **~23% total CPU** on the M5 Max (48 GB), load average ~3, package temp ~181°F. A naive `ps -A -o %cpu | awk` sum reads **~460–716% "summed CPU"** — sounds huge, but that's only ~23% of the machine; in a per-core view (Stats menubar) the performance cores were moderately filled and the **efficiency cores nearly idle**. Tripling 20→60 engines only ~2× the summed CPU and the machine stayed far from saturation; a single BFS level (~13k boards) took 3+ minutes.

**Root cause (Jason flagged "stdio isn't THIS slow; something else is strongly inefficient" — correct).** `ExternalEnginePlayer._read_line` (`gomoku/external_engine.py`) spawns a **brand-new `threading.Thread` for every single line read** (a `t.start()/t.join(timeout)` watchdog to enforce the read deadline). A multiPV analysis emits dozens-to-hundreds of `INFO PV/EVAL/WINRATE/BESTLINE` lines per board (every PV re-emitted at each iterative-deepening depth). So each board is a storm of short-lived thread create/start/join calls, **each taking the GIL** — and with 60 `analyze_states` worker threads doing this concurrently, the single Python coordinator serializes on the GIL doing thread-spawn machinery instead of reading. The 60 Rapfi *processes* finish their search quickly, then **block waiting for the GIL-bound Python reader to drain their pipe one freshly-threaded line at a time.** The bottleneck is the Python read path, not the engines and not the pipe.

**This is a measurement caution, not a property of the machine: the 23%-CPU "light footprint" is artificial.** Do NOT cite this run as evidence that a Rapfi pool is inherently low-CPU / safely co-tenant — it is throttled by the read-path bug. Fix: replace the per-line watchdog thread with an fd-level timed read (`select.select([stdout], [], [], timeout)` then `readline()`) — no thread, releases the GIL during the wait, so all 60 engines run in parallel and the bottleneck moves back to Rapfi (CPU should climb sharply). Re-measure the true Rapfi-pool footprint and the [[eval-teacher-sensei]] co-tenancy headroom AFTER that fix; the pre-fix number understates real CPU use.
