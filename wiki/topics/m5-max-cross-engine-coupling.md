...
### Rapfi-pool footprint — 60 latency-bound CPU engines = ~23% of the M5 Max (2026-06-24)

A warm pool of **60 Rapfi engines** driving the gomocup stdin/stdout protocol (15×15 NNUE, `--mine-bfs` BFS teacher-mining, `max_node=20000`, `max_pv=8`, pure-CPU — no GPU/MPS) lands at only **~23% total CPU** on the M5 Max (48 GB), load average ~3, package temp ~181°F. A naive `ps -A -o %cpu | awk` sum reads **~460–716% "summed CPU"** — sounds huge, but that's only ~23% of the machine; in a per-core view (Stats menubar) the performance cores were moderately filled and the **efficiency cores nearly idle**. Earlier in the same run, 20 engines gave ~350% summed / ~21 boards/s; tripling to 60 roughly doubled summed CPU but the machine stayed far from saturation. **Interpretation: this workload is latency / IO-round-trip-bound, not compute-bound.** Each board analysis is a stdin/stdout exchange (RESTART/BOARD/DONE/YXNBEST + readline round-trips) where engines spend much of their time waiting on dispatch rather than pinning cores — deep boards (10 stones, top-8 PV cap) search fast, so the ceiling is per-analyze round-trip latency × boards / parallelism, not raw cores. **This sharpens the [[eval-teacher-sensei]] "CPU-only sensei never competes with the MPS trainer" claim:** it's non-competing not only because it's a different engine but because it's *light* — a latency-bound CPU load that leaves the perf cores mostly free and the efficiency cores idle draws little package power (unlike the synthetic GPU hog above), implying large co-tenancy headroom for running a Rapfi pool alongside an MPS trainer (subject to the shared-package-power caveat documented above).
